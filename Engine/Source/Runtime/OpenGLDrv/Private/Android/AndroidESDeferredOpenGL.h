...
struct FAndroidESDeferredOpenGL : public FOpenGLESDeferred
{
	...
	static FORCEINLINE void DeleteSync(UGLsync Sync)
	{
		...
		{ 
			...
			{
				//handle error here
				EGLBoolean Result = eglDestroySyncKHR(AndroidEGL::GetInstance()->GetDisplay(), Sync.EGL);
				if (Result == EGL_FALSE)
				{
					//handle error here
				}
	...
	static FORCEINLINE UGLsync FenceSync(GLenum Condition, GLbitfield Flags)
	{
		if (IsES2())
		{
			...
			return UGLsync(GUseThreadedRendering ? eglCreateSyncKHR(AndroidEGL::GetInstance()->GetDisplay(), EGL_SYNC_FENCE_KHR, NULL) : 0);
		}
		else
		{
	...
	static FORCEINLINE EFenceResult ClientWaitSync(UGLsync Sync, GLbitfield Flags, GLuint64 Timeout)
	{
		if (IsES2())
		{
			if (GUseThreadedRendering)
			{
				...
				GLenum Result = eglClientWaitSyncKHR(AndroidEGL::GetInstance()->GetDisplay(), Sync.EGL, EGL_SYNC_FLUSH_COMMANDS_BIT_KHR, Timeout);
				switch (Result)
				{
