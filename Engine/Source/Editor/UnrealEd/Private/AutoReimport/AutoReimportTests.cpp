...
{
	return FPaths::ConvertRelativePathToFull(FPaths::EngineContentDir()) / TEXT("NotForLicensees") / TEXT("Automation") / TEXT("EditorAutomation") / TEXT("AutoReimportTestSource") / TEXT("");
}
...
bool CopyTestFiles(const FAutoReimportTestPayload& Test, const TArray<FSrcDstFilenames>& Files)
{
	...
	const auto SourceDir = GetTestSourceDir();
