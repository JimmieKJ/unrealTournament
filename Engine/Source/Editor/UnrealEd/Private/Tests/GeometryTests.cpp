...
bool FGeometryPromotionValidation::RunTest(const FString& Parameters)
{
	...
	TestEqual<int32>(TEXT("The number of Additive BSP brushes created is not correct, expected 9."), AdditiveBSP.Num(), 9);
	TestEqual<int32>(TEXT("The number of Additive BSP brushes created is not correct, expected 2."), SubtractiveBSP.Num(), 2);
...
bool FBSPCubePlacement::RunTest(const FString& Parameters)
{
	...
	// Verify the BSP brushes that were created have been found.
	TestEqual<int32>(TEXT("The number of BSP brushes created is not correct, expected 2."), BrushCount, 2);
	...
	TestEqual<uint32>(TEXT("The number of vertices are incorrect for the additive brush."), AdditiveBSP->Brush->NumUniqueVertices, CUBE_VERTS_COUNT);
	TestEqual<uint32>(TEXT("The number of vertices are incorrect for the subtractive brush."), SubtractiveBSP->Brush->NumUniqueVertices, CUBE_VERTS_COUNT);

	// Verify there are the correct amount of BSP surfaces are visible in the level.
	TestEqual<int32>(TEXT("Incorrect total number of surfaces are being reported, expected 10."), BSPModel->Surfs.Num(), 10);
