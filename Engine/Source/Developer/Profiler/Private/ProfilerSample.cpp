...
FString EProfilerSampleTypes::ToName( Type ProfilerSampleType )
{
	...
	{
		case HierarchicalTime: return LOCTEXT("StatType_Name_Hierarchical","Hier").ToString();
		case NumberInt: return LOCTEXT("StatType_Name_NumberInt","Int").ToString();
		case NumberFloat: return LOCTEXT("StatType_Name_NumberFloat","Float").ToString();
		case Memory: return LOCTEXT("StatType_Name_Memory","Mem").ToString();

		default: return LOCTEXT("StatType_InvalidOrMax","Unknown profiler sample type").ToString();
	}
}
...
FString EProfilerSampleTypes::ToDescription( Type ProfilerSampleType )
{
	switch( ProfilerSampleType )
	{
		...
		default: return LOCTEXT("StatDesc_InvalidOrMax","Unknown profiler sample type").ToString();
	}
}
