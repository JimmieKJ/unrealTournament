...
void KeyState::HandleEvent( double const time, bool const down, int const repeatCount )
{
	...
	if ( wasDown == down )
	{
		...
		// war - commented this assert out as it is causing a crash in debug builds when the back key is spammed going in and out of PlatformActivity from SDKExamples
		PendingEvent = KEY_EVENT_NONE;
		return;
	}
