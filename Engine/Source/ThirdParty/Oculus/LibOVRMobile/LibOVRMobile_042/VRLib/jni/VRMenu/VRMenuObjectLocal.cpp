...
Bounds3f VRMenuObjectLocal::GetTextLocalBounds( BitmapFont const & font ) const
{
    if ( TextDirty )
    {
		...
		else
		{
			...
			font.CalcTextMetrics( Text.ToCStr(), len, TextMetrics.w, TextMetrics.h, 
					TextMetrics.ascent, TextMetrics.descent, lineWidths, MAX_LINES, numLines );
		}
    }
	...
	float const vofs = ( TextMetrics.h * 0.5f ) - TextMetrics.ascent;
	Vector3f trans( TextMetrics.w * -0.5f * (int)FontParms.CenterHoriz, 
					vofs * (int)FontParms.CenterVert, 
					0.0f );
...
void VRMenuObjectLocal::RegenerateSurfaceGeometry( int const surfaceIndex )
{
	if ( surfaceIndex < 0 || surfaceIndex >= Surfaces.GetSizeI() )
	{
		DROID_ASSERT( surfaceIndex >= 0 && surfaceIndex < Surfaces.GetSizeI(), "VrMenu" );
