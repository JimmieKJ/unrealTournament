		...
		Vector3f const & scale, float & t0, float & t1 ) const
{
	...
	RtIntersect::RayBounds( start, dir, scaledBounds.GetMins(), scaledBounds.GetMaxs(), t0, t1 );
		...
		Vector3f const & scale, ContentFlags_t const testContents, OvrCollisionResult & result ) const
{
    ...
    for ( int i = 0; i < Indices.GetSizeI(); i += 3 )
    {
        ...
        if ( RtIntersect::RayTriangle( localStart, localDir, verts[0], verts[1], verts[2], t_, u_, v_ ) )
        {
            if ( t_ < result.t )
            {
                result.t = t_;
