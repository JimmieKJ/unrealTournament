...
{
	ovr_ReadFileFromOtherApplicationPackage( packageZipFile, nameInZip, length, buffer );
}

// This is DEFINITELY NOT thread safe! 
void ovr_ReadFileFromOtherApplicationPackage( void * zipFile, const char * nameInZip, int & length, void * & buffer )
{
	length = 0;
	buffer = NULL;
	if ( zipFile == 0 )
