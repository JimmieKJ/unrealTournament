...
private:
                                 ...
                                 VRMenuObject * self, VRMenuEvent const & event )
	{
		...
		const char* dndStrName;
		if( enabled )
		{
			dndStrName = "@string/do_not_disturb_on";
		}
		else
		{
			dndStrName = "@string/do_not_disturb_off";
		}
		String dndStr;
		VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), dndStrName, dndStrName, dndStr );
		self->SetText( dndStr );
...
private:
                                 ...
                                 VRMenuObject * self, VRMenuEvent const & event )
	{
		...
		const char* comfortModeStrName;
		if( enabled )
		{
			comfortModeStrName = "@string/comfort_mode_on";
		}
		else
		{
			comfortModeStrName = "@string/comfort_mode_off";
		}
		String comfortModeStr;
		VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), comfortModeStrName, comfortModeStrName, comfortModeStr );
		self->SetText( comfortModeStr );
...
public:
	...
	OvrReorient_OnUp( OvrGlobalMenu & globalMenu_ ) :
		GlobalMenu( globalMenu_ ),
		IsTutorial( false )		
	{ 
	}

	void				SetIsTutorial( bool const isTutorial ) { IsTutorial = isTutorial; }
...
private:
                                 ...
                                 VRMenuObject * self, VRMenuEvent const & event )
	{
		...
		if ( !IsTutorial )
		{
			app->RecenterYaw( true ); // This is only done on the current app - ie. PlatformActivity
			...
			// Notify the underlying activity of the reorient if one is specified
			if ( !GlobalMenu.LaunchingPackage().IsEmpty() )
			{
				app->SendIntent( GlobalMenu.LaunchingPackage().ToCStr(), "reorient ", EXIT_TYPE_FINISH );
			}
			else
			{
				app->ExitPlatformUI();
...
OvrGlobalMenu * OvrGlobalMenu::Create( App * app, OvrVRMenuMgr & menuMgr, BitmapFont const & font )
{
    ...
    String reachingGlobalMenuStr;
    String titleHomeStr;
    String instrHomeStr;
    String titleReorientStr;
    String instrReorientStr;
    String titleBrightnessStr;
    String instrBrightnessStr;
    String titleDoNotDisturbStr;
    String instrDoNotDisturbStr;
    String titleComfortModeStr;
    String instrComfortModeStr;
    String titlePassthroughCameraStr;
    String instrPassthroughCameraStr;
    String titleBackBtnStr;
    String instrBackBtnStr;

	VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/instr_reaching_global_menu"	, "@string/instr_reaching_global_menu"	, reachingGlobalMenuStr 	);
	VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/title_home"					, "@string/title_home"					, titleHomeStr 				);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/instr_home"					, "@string/instr_home"					, instrHomeStr 				);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/title_reorient"				, "@string/title_reorient"				, titleReorientStr 			);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/instr_reorient"				, "@string/instr_reorient"				, instrReorientStr 			);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/title_brightness"				, "@string/title_brightness"			, titleBrightnessStr 		);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/instr_brightness"				, "@string/instr_brightness"			, instrBrightnessStr 		);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/title_do_not_disturb"			, "@string/title_do_not_disturb"		, titleDoNotDisturbStr 		);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/instr_do_not_disturb"			, "@string/instr_do_not_disturb"		, instrDoNotDisturbStr 		);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/title_comfort_mode"			, "@string/title_comfort_mode"			, titleComfortModeStr 		);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/instr_comfort_mode"			, "@string/instr_comfort_mode"			, instrComfortModeStr 		);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/title_passthrough_camera"		, "@string/title_passthrough_camera"	, titlePassthroughCameraStr );
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/instr_passthrough_camera"		, "@string/instr_passthrough_camera"	, instrPassthroughCameraStr );
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/title_back_btn"				, "@string/title_back_btn"				, titleBackBtnStr 			);
    VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/instr_back_btn"				, "@string/instr_back_btn"				, instrBackBtnStr 			);

	float const ITEM_TEXT_SCALE = 1.0f;

    font.WordWrapText( reachingGlobalMenuStr	, 1.4f, ITEM_TEXT_SCALE );
	font.WordWrapText( instrHomeStr				, 1.0f, ITEM_TEXT_SCALE );
	font.WordWrapText( instrReorientStr			, 1.0f, ITEM_TEXT_SCALE );
	font.WordWrapText( instrBrightnessStr		, 1.0f, ITEM_TEXT_SCALE );
	font.WordWrapText( instrDoNotDisturbStr		, 1.0f, ITEM_TEXT_SCALE );
	font.WordWrapText( instrComfortModeStr		, 1.0f, ITEM_TEXT_SCALE );
	font.WordWrapText( instrPassthroughCameraStr, 1.0f, ITEM_TEXT_SCALE );
	font.WordWrapText( instrBackBtnStr			, 1.0f, ITEM_TEXT_SCALE );

    String homeStr 				= titleHomeStr 				+ "\n" + instrHomeStr;
    String reorientStr 			= titleReorientStr 			+ "\n" + instrReorientStr;
	String brightnessStr 		= titleBrightnessStr 		+ "\n" + instrBrightnessStr;
	String doNotDisturbStr 		= titleDoNotDisturbStr 		+ "\n" + instrDoNotDisturbStr;
	String comfortModeStr 		= titleComfortModeStr 		+ "\n" + instrComfortModeStr;
	String passthroughCameraStr = titlePassthroughCameraStr	+ "\n" + instrPassthroughCameraStr;
	String backBtnStr 			= titleBackBtnStr 			+ "\n" + instrBackBtnStr;
	...
	{
		VRMenuFontParms itemFontParms( true, true, false, false, true, 0.525f, 0.475f, ITEM_TEXT_SCALE );
...

		{
			String textOff;
			String textOn;
			VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/do_not_disturb_on"	, "@string/do_not_disturb_on" , textOn );
			VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/do_not_disturb_off", "@string/do_not_disturb_off", textOff );
		...
		{
			String text;
			VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/reorient", "@string/reorient", text );
			...
			Array< VRMenuComponent* > comps;
			comps.PushBack( new OvrDefaultComponent( Vector3f( 0.0f, 0.0f, 0.05f ), 1.0f, 0.125f, 0.125f, textColor, textHilightColor ) );
			comps.PushBack( new OvrReorient_OnUp( *menu ) );
			//comps.PushBack( new OvrTextFade_Component( iconBaseOffset, iconFadeOffset ) );
		...
		{
			String text;
			VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/oculus_home", "@string/oculus_home", text );
		...
		{
			String text;
			VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/brightness", "@string/brightness", text );
...

		{
			String textOn;
			String textOff;
			VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/comfort_mode_on", "@string/comfort_mode_on", textOn );
			VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/comfort_mode_off", "@string/comfort_mode_off", textOff );
...

		{
			String textOn;
			String textOff;
			VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/passthrough_camera_on", "@string/passthrough_camera_on", textOn );
			VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), "@string/passthrough_camera_off", "@string/passthrough_camera_off", textOff );
        ...
        BitmapFontSurface & fontSurface, gazeCursorUserId_t const gazeUserId )
{
	...
	// update passthrough camera text because mounting on and sleeping may turn the camera off
	{
		...
		if ( obj != NULL )
		{
			...
			if( enabled )
			{
				strName = "@string/passthrough_camera_on";
			}
			else
			{
				strName = "@string/passthrough_camera_off";
			}
			String str;
			VrLocale::GetString( app->GetVrJni(), app->GetJavaObject(), strName, strName, str );
			obj->SetText( str );
		}
	}
}
