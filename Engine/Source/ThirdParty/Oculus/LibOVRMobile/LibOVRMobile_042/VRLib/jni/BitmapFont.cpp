...
float BitmapFontLocal::CalcTextWidth( char const * text ) const
{
	...
	for ( uint32_t charCode = UTF8Util::DecodeNextChar( &p ); *p != '\0' && charCode != '\0'; charCode = UTF8Util::DecodeNextChar( &p ) )
	{
        ...
        FontGlyphType const & g = GlyphForCharCode( charCode );
		width += g.AdvanceX * FontInfo.ScaleFactor;
    }
    return width;
}
		...
		float & firstAscent, float & lastDescent, float * lineWidths, int const maxLines, int & numLines ) const
{
	...
	for ( ; ; len++ )
	{
		...
		FontGlyphType const & g = GlyphForCharCode( charCode );
		lineWidths[numLines] += g.AdvanceX * FontInfo.ScaleFactor;
	...
	firstAscent *= FontInfo.ScaleFactor;
	lastDescent *= FontInfo.ScaleFactor;
	height = firstAscent;
	height += ( numLines - 1 ) * FontInfo.FontHeight * FontInfo.ScaleFactor;
	height += lastDescent;
