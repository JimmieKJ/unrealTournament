...
static OvrConsole Console;

}	// namespace Ovr

// add a pointer to a function that can be executed from the console
void ovr_RegisterConsoleFunction( const char * name, consoleFn_t function )
{
	OVR::Console.RegisterConsoleFunction( name, function );
}

void ovr_UnRegisterConsoleFunctions()
{
	OVR::Console.UnRegisterConsoleFunctions();
}
...
JNIEXPORT void Java_com_oculusvr_vrlib_VrLib_nativeVolumeEvent(JNIEnv *jni, jclass clazz, jint volume)
{
    ...
    {
    	// Unity apps generate a volume change on startup, causing the volume menu
    	// to show up when they haven't changed the volume, so ignore first volume change.
    	// This means that Native apps the user has to press the volume twice before it
...
{
	char const * commandStr = jni->GetStringUTFChars( command, NULL );
...
void ovr_OnLoad( JavaVM * JavaVm_ )
{
	...
	ovr_SetVrActivityClass( ovr_GetGlobalClassReference( jni, "com/oculusvr/vrlib/VrActivity" ) );
	...
	// where we are loaded as a secondary .so.
	{
		static JNINativeMethod gMethods[] = {
				{"nativeDockEvent", "(I)V",(void*)Java_com_oculusvr_vrlib_DockReceiver_nativeDockEvent },
		};
		if ( JNI_OK != jni->RegisterNatives( DockReceiverClass, gMethods, sizeof(gMethods)/sizeof(gMethods[0])) )
		{
			FAIL( "RegisterNatives failed" );
		}
	}
	{
		static JNINativeMethod gMethods[] = {
				{"nativeProximitySensor", "(I)V",(void*)Java_com_oculusvr_vrlib_ProximityReceiver_nativeProximitySensor },
		};
		if ( JNI_OK != jni->RegisterNatives( ProximityReceiverClass, gMethods, sizeof(gMethods)/sizeof(gMethods[0])) )
		{
			FAIL( "RegisterNatives failed" );
		}
		LOG( "Registered proximity receiver");
	}
	{
		static JNINativeMethod gMethods[] = {
				{"nativeVolumeEvent", "(I)V",(void*)Java_com_oculusvr_vrlib_VrLib_nativeVolumeEvent },
				{"nativeBatteryEvent", "(III)V",(void*)Java_com_oculusvr_vrlib_VrLib_nativeBatteryEvent },
				{"nativeHeadsetEvent", "(I)V",(void*)Java_com_oculusvr_vrlib_VrLib_nativeHeadsetEvent },
				{"nativeWifiEvent", "(II)V",(void*)Java_com_oculusvr_vrlib_VrLib_nativeWifiEvent },
				{"nativeCellularStateEvent", "(I)V",(void*)Java_com_oculusvr_vrlib_VrLib_nativeCellularStateEvent },
				{"nativeCellularSignalEvent", "(I)V",(void*)Java_com_oculusvr_vrlib_VrLib_nativeCellularSignalEvent },
				{"nativeVsync", "(J)V",(void*)Java_com_oculusvr_vrlib_VrLib_nativeVsync },
		};
		if ( JNI_OK != jni->RegisterNatives( VrLibClass, gMethods, sizeof(gMethods)/sizeof(gMethods[0])) )
		{
			FAIL( "RegisterNatives failed" );
		}
		LOG( "Registered proximity receiver");
...
//
// This calls capi ovr_Initialize() internally.
void ovr_Init()
{
	...
	ovr_RegisterConsoleFunction( "print", DebugPrint );
}
...
void ovr_ExitActivity( ovrMobile * ovr, eExitType exitType )
{
	...
	else if ( exitType == EXIT_TYPE_EXIT )
	{
		...
		ovr_UnRegisterConsoleFunctions();
...
void ovr_AbandonAudioFocus( ovrMobile * ovr )
{
	if ( ovr == NULL )
	{
		return;
