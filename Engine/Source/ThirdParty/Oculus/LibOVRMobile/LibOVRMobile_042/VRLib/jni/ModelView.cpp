...
void OvrSceneView::UpdateViewMatrix(const VrFrame vrFrame )
{
	...
	Vector3f shiftedEyePos = CenterEyePos();

	Vector3f headModelOffset = HeadModelOffset( EyeRoll, EyePitch, EyeYaw,
			ViewParms.HeadModelLength, ViewParms.HeadModelAngle );
	if ( useHeadModel )
	{
	...
	{
LOG( "%f %f %f",
		vrFrame.PoseState.Pose.Position.x,
		vrFrame.PoseState.Pose.Position.y,
		vrFrame.PoseState.Pose.Position.z );
...
void OvrSceneView::UpdateSceneModels( const VrFrame vrFrame )
{
