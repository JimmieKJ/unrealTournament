...
bool SensorFusion::AttachToSensor(SensorDevice* sensor)
{
    ...
    if (sensor != NULL)
    {
        SensorDataAvailable = sensor->IsConnected();
...
void SensorFusion::RecenterYaw()
{
	...
	// find the yaw in the current state
	float yaw, pitch, roll;
	state.State.Transform.Orientation.GetEulerAngles< Axis_Y, Axis_X, Axis_Z >( &yaw, &pitch, & roll );

	Vector3f translation( 0.0f );	// there is no positional tracking yet, so we don't recenter position
	Posef yawOnlyPose( Quatf( Axis_Y, yaw ), translation );
	RecenterTransform = yawOnlyPose.Inverted();
}
...
void SensorFusion::applyMagYawCorrection(const Vector3f& magUncalibrated, const Vector3f& magBias, const Vector3f& gyro, float deltaT)
{
    ...
    const float proportionalGain = 0.01f;
    const float integralGain     = 0.0005f;	
    const float correctionRadPerSecMax = 0.07f * Mathf::DegreeToRadFactor;
    ...
    if (MagRefIdx >= 0)
    {
        ...
        float totalCorrectionRadPerSec = propCorrectionRadPerSec + MagCorrectionIntegralTerm;
        ...
        MagCorrectionIntegralTerm += yawError * integralGain * deltaT;
		...
		if (logCount % 100 == 0)
		{
			...
			LogText("YAW - [%d:%d] ypr=%.1f,%.1f,%.1f ref_ypr=%.1f,%.1f,%.1f magUncalib=%.3f %.3f %.3f magBias=%.3f %.3f %.3f error=%.3f prop=%.6f integ=%.6f correction=%.6f\n",
					MagRefIdx,
					MagRefs.GetSize(),
					yaw, pitch, roll,
					pyaw, ppitch, proll,
					...
					MagCorrectionIntegralTerm * Mathf::RadToDegreeFactor,
...
{		
    SensorState	sstate;
    sstate.Status = SensorDataAvailable ? Status_OrientationTracked : 0;
...
{    
    ...
    {
        pFusion->SensorDataAvailable = true;
    }
    else if (msg.Type == Message_DeviceRemoved)
    {
        pFusion->SensorDataAvailable = false;
    }
}
