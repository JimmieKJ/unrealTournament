...
public:
    ...
    explicit Thread(UPInt stackSize = 128 * 1024, int processor = -1);
    ...
    // "processor" parameter specifies which hardware processor this thread will be run on. 
    // -1 means OS decides this. Implemented only on Win32
    explicit Thread(ThreadFn threadFunction, void*  userHandle = 0, UPInt stackSize = 128 * 1024,
...
{
public:
		ThreadRefCounted(UPInt stackSize = 128 * 1024, int processor = -1) : Thread( stackSize, processor ) {}
		ThreadRefCounted(ThreadFn threadFunction, void*  userHandle = 0, UPInt stackSize = 128 * 1024,
