...
const float OvrVRMenuMgr::DEFAULT_TEXT_SCALE = 0.0025f;
...
public:
	...
	virtual void				RenderSubmitted( Matrix4f const & mvp );

    virtual GlProgram *         GetGUIGlProgram( eGUIProgramType const programType );
...
private:
	...
	void						SubmitForRenderingRecursive( OvrDebugLines & debugLines, BitmapFont const & font, 
                                        ...
                                        SubmittedMenuObject * submitted, int const maxIndices, int & curIndex ) const;
        ...
        Bounds3f & cullBounds, SubmittedMenuObject * submitted, int const maxIndices, int & curIndex ) const
{
	...
	Posef curModelPose;
	curModelPose.Position = parentModelPose.Position + ( parentModelPose.Orientation * localPose.Position );
	curModelPose.Orientation = localPose.Orientation * parentModelPose.Orientation;

	Vector4f curColor = parentColor * obj->GetColor();
	Vector3f const & localScale = obj->GetLocalScale() * obj->GetHilightScale();
...
/*
	...
	if ( parent != NULL )
	{
		...
		fontSurface.DrawText3D( font, fontParms, curModelPose.Position, itemNormal, itemUp, 
				DEFAULT_TEXT_SCALE, Vector4f( 1.0f, 0.0f, 1.0f, 1.0f ), parent->GetText().ToCStr() );
	}
*/
	if ( obj->GetType() != VRMENU_CONTAINER )	// containers never render, but their children may
	{
        ...
        Posef itemPose( curModelPose.Orientation * hilightPose.Orientation,
                        curModelPose.Position + ( curModelPose.Orientation * hilightPose.Position ) );
		...
		if ( ( oFlags & VRMENUOBJECT_DONT_RENDER_TEXT ) == 0 && text.GetLengthI() > 0 )
		{
            ...
            Posef curTextPose;
            curTextPose.Position = itemPose.Position + ( itemPose.Orientation * textLocalPose.Position );
    ...
    if ( obj->Children.GetSizeI() > 0 )
    {
	    for ( int i = 0; i < obj->Children.GetSizeI(); ++i )
	    {
            ...
            childCullBounds = Bounds3f::Transform( child->GetLocalPose(), childCullBounds );
    ...
    {
        Bounds3f localBounds = obj->GetLocalBounds( font );
        ...
        textLocalBounds = Bounds3f::Transform( obj->GetHilightPose(), textLocalBounds );
    	debugLines.AddBounds( curModelPose, textLocalBounds, Vector4f( 1.0f, 1.0f, 0.0f, 1.0f ) );
    }
#endif
}
        ...
        VRMenuRenderFlags_t const & flags )
{
	...
	{
		LOG( "Too many menu objects submitted!" );
		return;
	}
	...
	SubmitForRenderingRecursive( debugLines, font, fontSurface, flags, obj, worldPose, Vector4f( 1.0f ), 
			Vector3f( 1.0f ), cullBounds, Submitted, MAX_SUBMITTED, NumSubmitted );
}
...
void VRMenuMgrLocal::RenderSubmitted( Matrix4f const & worldMVP )
{
	if ( NumSubmitted == 0 )
	{
		return;
	...
	for ( int i = 0; i < NumSubmitted; ++i )
	{
		...
		if ( obj != NULL )
		{
			...
			Matrix4f transform( Posef( cur.Pose.Orientation, translation ) );
...
GlProgram * VRMenuMgrLocal::GetGUIGlProgram( eGUIProgramType const programType )
{
    switch( programType )
    {
