...
OvrVolumePopup * OvrVolumePopup::Create( App * app, OvrVRMenuMgr & menuMgr, BitmapFont const & font )
{
...

	{
		...
		VRMenuFontParms fontParms( false, true, false, false, false, 0.5f );
	...
	menu->InitWithItems( menuMgr, font, 1.8f, VRMenuFlags_t( VRMENU_FLAG_TRACK_GAZE ), parms );
        ...
        BitmapFontSurface & fontSurface, gazeCursorUserId_t const gazeUserId )
{
	...
	const double remaining = VolumeFadeTime - now;
	if ( remaining <= 0 )
	{
...
void OvrVolumePopup::ShowVolume( App * app, const int current, const int max )
{
	VolumeFadeTime = TimeInSeconds() + VolumeMenuFadeDelay;
