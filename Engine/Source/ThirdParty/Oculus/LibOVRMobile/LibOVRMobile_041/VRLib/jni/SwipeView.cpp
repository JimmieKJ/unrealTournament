...
// of the matrix, or flase if the two points don't cross the Z=0 plane.
...
#if 0	// refactoring work in progress

enum SwipeViewAction
{
	ACTION_NONE,
	DRAG,
	COAST,
...
struct Transitions
{
	SwipeViewTouchState		NewState;
	SwipeViewAction			Action;
};

eGazeCursorStateType	SwipeView::GazeCursorType() const
{
	bool	buttonState;	// up, down, downDragged
	bool	overPanel;

	// always play panel select / deselect on panel transition

	// always reset TouchPoint and PressTime when button down

	static const Transitions	transitions[SVT_NUM_STATES][4] =
	{			//	up + noPanel				up + panel					down + noPanel		down + panel
/*SVT_WILL_DRAG*/	{ {-1,0},					{SVT_WILL_ACTION, 0},		{SVT_DRAG, 0},		{SVT_ACTION,0} },
/*SVT_WILL_ACTION*/	{ {SVT_WILL_DRAG,0},		{-1,0},						{SVT_DRAG, 0},		{SVT_ACTION,0} },
/*SVT_DRAG*/		{ {SVT_WILL_DRAG,COAST},	{SVT_WILL_ACTION,COAST},	{-1,DRAG},			{-1,DRAG} },
/*SVT_ACTION*/		{ {SVT_WILL_DRAG,0},		{SVT_WILL_ACTION,ACTION},	{SVT_DRAG,0},		{-1,0} },
	};


	// If on an active panel, button is down, and touchState is action
	// release will activate.

	// if on an active panel, button is up, and velocity is low
	// press will go to provisional-activate

	// if button is down and touchState is drag
	// release will go to coast mode
}
        ...
        const VrFrame & vrFrame, const Matrix4f & view, const bool allowSwipe )
{
	...
	if ( State != SVS_CLOSED )
	{
		...
		const Vector3f rayTraceP1 = MatrixOrigin( view );
...
//			Vector3f textPos = pMat.GetZBasis() * ( animatedDistance + 0.08f ) + StartViewOrigin - ( pMat.GetYBasis() * 1.1f );
			...
			fontParms_t parms;
			parms.CenterHoriz = true;
			parms.CenterVert = true;
			parms.Billboard = false;

			if ( index == SelectedPanel )
			{
				fontSurface.DrawText3D( font, parms, textPos, textNormal, textUp,
						0.0025f, Vector4f( 1.0f, 1.0f, 1.0f, panel.SelectState ), panel.Text );
