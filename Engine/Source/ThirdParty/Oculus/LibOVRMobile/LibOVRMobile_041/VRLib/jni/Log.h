...
#define LOG( ... ) LogWithFileTag( ANDROID_LOG_WARN, __FILE__, __VA_ARGS__ )
#define FAIL( ... ) {LogWithFileTag( ANDROID_LOG_WARN, __FILE__, __VA_ARGS__ );abort();}
...
#define SPAM(...) __android_log_print( ANDROID_LOG_VERBOSE, "Spam", __VA_ARGS__ )
#else
#define SPAM(...) { }
#endif
...
#if !defined( OVR_BUILD_DEBUG )
#define DROID_ASSERT( __expr__, __tag__ ) { if ( !( __expr__ ) ) { DROIDLOG( __tag__, "ASSERTION FAILED: %s", #__expr__ ); } }
#else
#define DROID_ASSERT( __expr__, __tag__ ) { if ( !( __expr__ ) ) { DROIDLOG( __tag__, "ASSERTION FAILED: %s", #__expr__ ); OVR_DEBUG_BREAK; } }
#endif
...
public:
	...
	~LogCpuTime()
	{
		...
		DROIDLOG( "LogCpuTime", "%s took %6.4f seconds", Label, ( endTimeNanoSec - StartTimeNanoSec ) * ( 1.0 / 1000000000.0 ) );
	}
	...
	static double GetNanoSeconds()
	{
		...
		return (double)now.tv_sec * 1000000000LL + now.tv_nsec;
	}
...
// Qualcomm drivers. Disable by default, but allow
