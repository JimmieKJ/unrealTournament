...
void OvrGuiSysLocal::RepositionMenus( App * app )
{
...
void OvrGuiSysLocal::AddMenu( VRMenu * menu )
{
	...
	if ( menuIndex >= 0 )
	{
		LOG( "Duplicate menu name '%s'", menu->GetName() );
...
//==============================
// OvrGuiSysLocal::RemoveMenu
void OvrGuiSysLocal::RemoveMenu( VRMenu * menu )
{
...
void OvrGuiSysLocal::Shutdown( OvrVRMenuMgr & menuMgr )
{
	...
	// no way to call GuiSys->RemoveMenu() for them.
	for ( int i = 0; i < Menus.GetSizeI(); ++i )
	{
		VRMenu * menu = Menus[i];
        ...
        BitmapFontSurface & fontSurface )
{
	...
	for ( int i = ActiveMenus.GetSizeI() - 1; i >= 0; --i )
	{
		...
		if ( 0 )
		{
			...
			{
				menuMgr.BeginFrame();
				curMenu->Frame( app, vrFrame, menuMgr, font, fontSurface, GazeUserId );
			}
