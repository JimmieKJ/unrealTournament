...
static TextureFormatMap TextureFormatNames[] =
{
    ...
    { Texture_ASTC_6_5,		"Texture_ASTC_6_5" },
...
static size_t GetOvrTextureSize( const int format, const int w, const int h )
{
    switch ( format & Texture_TypeMask )
    {
        ...
        case Texture_ASTC_6_5:
		{
            int bw = (w+5)/6, bh = (h+4)/5;
        	return bw * bh * 16;
        }
        default:
		{
						...
						const int mipcount, const bool useSrgbFormat, const bool imageSizeStored )
{
	...
	{
		size_t mipSize = GetOvrTextureSize( format, w, h );
		if ( imageSizeStored )
		{
						...
						const int mipcount, const bool useSrgbFormat, const bool imageSizeStored )
{
	...
	{
		const int w = width >> i;
		size_t mipSize = GetOvrTextureSize( format, w, w );
		if ( imageSizeStored )
		{
			mipSize = *(const size_t *)level;
			level += 4;
						...
						bool useSrgbFormat, bool noMipMaps, int & width, int & height )
{
	...
	if ( ( startTex < sizeof( OVR_PVR_HEADER ) ) || ( startTex >= bufferLength ) )
	{
						...
						bool useSrgbFormat, bool noMipMaps, int & width, int & height )
{
	...
	const int startTex = sizeof( OVR_KTX_HEADER ) + header.bytesOfKeyValueData;
	if ( ( startTex < sizeof( OVR_KTX_HEADER ) ) || ( startTex >= bufferLength ) )
	{
