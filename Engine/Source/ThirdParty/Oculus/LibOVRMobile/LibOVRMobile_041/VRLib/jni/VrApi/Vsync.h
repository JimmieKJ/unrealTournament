...
// The Java_com_oculusvr_vrlib_Vsync_nativeVsync() function is called by a java choreographer doFrame() callback
...
//	void Java_com_oculusvr_vrlib_Vsync_nativeVsync( JNIEnv *jni, jclass clazz, jlong nanoSeconds );
...
// Returns a time in seconds for the given frame, which is used to
// calculate delta-time for game frames.
double			FramePointTimeInSeconds( const double framePoint );

// Includes scale and bias to more accurately account for signal arrival
// and blanked lines.
...
// This should be the same as java's system.nanoTime(), which is what
// the Choreographer vsync timestamp is based on.
long long 		NanoTime();

// NanoTime() * 0.000000001
