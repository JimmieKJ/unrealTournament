...
DrawCounters RenderSurfaceList( const DrawSurfaceList & drawSurfaceList ) {
	...
	/*{
		const Matrix4f & viewMatrix = drawSurfaceList.viewMatrix;
		const Vector4f viewOrigin = GLTransform( viewMatrix.Transposed(), Vector4f( -viewMatrix.M[3][0], -viewMatrix.M[3][1], -viewMatrix.M[3][2], 0.0f ) );
		LOG( "viewOrigin = %f %f %f", viewOrigin.x, viewOrigin.y, viewOrigin.z );
	}*/
