...
#include "../RayTracer/RtIntersect.h"
...
bool VRMenuSurfaceTexture::LoadTexture( eSurfaceTextureType const type, char const * imageName, bool const allowDefault )
{
	...
	if ( Handle == 0 && allowDefault )
	{
		...
		LOG( "VRMenuSurfaceTexture::CreateFromImage: failed to load image '%s' - default loaded instead!", imageName );
	}
    OwnsTexture = true;
	return Handle != 0;
}
...
void VRMenuSurface::CreateImageGeometry( int const textureWidth, int const textureHeight, ContentFlags_t const contents )
{
	OVR_ASSERT( Geo.vertexBuffer == 0 && Geo.indexBuffer == 0 && Geo.vertexArrayObject == 0 );

	int const NUM_VERTS = 4;

	// create the vertices
	Vector3f const faceNormal( 0.0f, 0.0f, 1.0f );
	Vector3f up( 0.0f, 1.0f, 0.0f );
	Vector3f right = up.Cross( faceNormal );
	right.Normalize();

	// Simply translating the width and height into world units would be absolutely huge 
	// (1 texel would be 1 meter), so we apply a default scale here.
	up *= textureHeight * VRMenuObject::DEFAULT_TEXEL_SCALE;
	right *= textureWidth * VRMenuObject::DEFAULT_TEXEL_SCALE;

	VertexAttribs attribs;
	attribs.position.Resize( NUM_VERTS );
	attribs.color.Resize( NUM_VERTS );
	attribs.uv0.Resize( NUM_VERTS );
	attribs.uv1.Resize( NUM_VERTS );

	Vector4f color( 1.0f, 1.0f, 1.0f, 1.0f );

	// lower left
	attribs.position[0] = -right * 0.5f - up * 0.5f;
	attribs.uv0[0] = Vector2f( 0.0f, 1.0f );
	attribs.uv1[0] = Vector2f( 0.0f, 1.0f );
	attribs.color[0] = color;
	// upper left
	attribs.position[1] = -right * 0.5f + up * 0.5f;
	attribs.uv0[1] = Vector2f( 0.0f, 0.0f );
	attribs.uv1[1] = Vector2f( 0.0f, 0.0f );
	attribs.color[1] = color;
	// upper right
	attribs.position[2] = right * 0.5f + up * 0.5f;
	attribs.uv0[2] = Vector2f( 1.0f, 0.0f );
	attribs.uv1[2] = Vector2f( 1.0f, 0.0f );
	attribs.color[2] = color;
	// lower right
	attribs.position[3] = right * 0.5f - up * 0.5f;
	attribs.uv0[3] = Vector2f( 1.0f, 1.0f );
	attribs.uv1[3] = Vector2f( 1.0f, 1.0f );
	attribs.color[3] = color;
...
void VRMenuSurface::Render( OvrVRMenuMgr & menuMgr, Matrix4f const & mvp, SubmittedMenuObject const & sub ) const
{
	...
	GL_CheckErrors( "VRMeuSurface::Render - pre" );
	...
	switch( pt )
	{
		...
		case PROGRAM_MAX:
		{
			LOG( "Unsupported texture map combination." );
			return;
		}
		default:
		{
	...
	GL_CheckErrors( "VRMeuSurface::Render - post" );
}
...
void VRMenuSurface::CreateFromSurfaceParms( VRMenuSurfaceParms const & parms )
{
	...
	CreateImageGeometry( Textures[surfaceIdx].GetWidth(), Textures[surfaceIdx].GetHeight(), parms.Contents );

	Dims.x = Textures[surfaceIdx].GetWidth();
	Dims.y = Textures[surfaceIdx].GetHeight();
	...
	{
		LOG( "Invalid material combination -- either add a shader to support it or fix it." );
		ProgramType = PROGRAM_MAX;
	}
}
...
VRMenuObjectLocal::VRMenuObjectLocal( VRMenuObjectParms const & parms, 
    ...
    HilightPose( Quatf(), Vector3f( 0.0f, 0.0f, 0.05f ) ),
    HilightScale( 1.0f ),
        ...
        Vector3f const & mins, Vector3f const & maxs, ContentFlags_t const testContents, float & t0, float & t1 ) const
{
	...
	RtIntersect::RayBounds( start, dir, mins, maxs, t0, t1 );
	return t0 >= 0.0f && t1 >= 0.0f && t1 >= t0;
}

//==============================
// VRMenuObjectLocal::IntersectRay
bool VRMenuObjectLocal::IntersectRay( Vector3f const & localStart, Vector3f const & localDir, Bounds3f const & bounds, 
        float & bounds_t0, float & bounds_t1, ContentFlags_t const testContents, OvrCollisionResult & result ) const
{
	...
	if ( CollisionPrimitive != NULL )
	{
		CollisionPrimitive->IntersectRay( localStart, localDir, localScale, testContents, result );
	}
	...
	if (  GetType() != VRMENU_CONTAINER )
	{
		...
		for ( int i = 0; i < Surfaces.GetSizeI(); ++i )
		{
			if ( Surfaces[i].IsRenderable() )
			{
				...
				if ( Surfaces[i].IntersectRay( localStart, localDir, localScale, testContents, localResult ) )
				{
...
bool VRMenuObjectLocal::HitTest_r( App * app, OvrVRMenuMgr & menuMgr, BitmapFont const & font, 
		Posef const & parentPose, Vector3f const & rayStart, Vector3f const & rayDir, 
		ContentFlags_t const testContents, HitTestResult & result ) const
{
	...
	Posef modelPose;
	modelPose.Position = parentPose.Position + ( parentPose.Orientation * LocalPose.Position );
...
//        DROIDLOG( "Spam", "Cull hit = %s, t0 = %.2f t1 = %.2f", hitCullBounds ? "true" : "false", cullT0, cullT1 );
    ...
    if ( GetContents() & testContents )
    {
        if ( Flags & VRMENUOBJECT_BOUND_ALL )
        {
            ...
            Bounds3f localBounds = GetLocalBounds( font );
            float t0;
	        float t1;
        ...
        else
        {
	        ...
	        Bounds3f const & localBounds = GetLocalBounds( font );
            OVR_ASSERT( !localBounds.IsInverted() );

	        bool hit = IntersectRay( localStart, localDir, localBounds, selfT0, selfT1, testContents, cresult );
            if ( hit )
    	    {
            ...
            if ( !Text.IsEmpty() && GetType() != VRMENU_CONTAINER && ( Flags & VRMENUOBJECT_DONT_HIT_TEXT ) == 0 )
            {
                float textT0;
                float textT1;
                Bounds3f bounds = GetTextLocalBounds( font );
	...
	for ( int i = 0; i < Children.GetSizeI(); ++i )
	{
		...
		if ( child != NULL )
		{
			...
			bool intersected = child->HitTest_r( app, menuMgr, font, modelPose, rayStart, rayDir, testContents, childResult );
...
{
	HitTest_r( app, menuMgr, font, worldPose, rayStart, rayDir, testContents, result );
...
void VRMenuObjectLocal::RenderSurface( OvrVRMenuMgr & menuMgr, Matrix4f const & mvp, SubmittedMenuObject const & sub ) const
{
...
Bounds3f VRMenuObjectLocal::GetLocalBounds( BitmapFont const & font ) const { 
	...
	{
        GetTextLocalBounds( font );
		bounds = Bounds3f::Union( bounds, TextLocalBounds );
	}
...
Bounds3f VRMenuObjectLocal::GetTextLocalBounds( BitmapFont const & font ) const
{
    ...
    else
    {
	    ...
	    font.CalcTextMetrics( Text.ToCStr(), len, w, h, ascent, descent, lineWidths, MAX_LINES, numLines );

        // NOTE: despite being 3 scalars, text scaling only uses the x component since
        // DrawText3D doens't take separate x and y scales right now.
        Vector3f const localScale = GetLocalScale();
        Vector3f const textLocalScale = GetTextLocalScale();
        float const scale = localScale.x * textLocalScale.x * FontParms.Scale * OvrVRMenuMgr::DEFAULT_TEXT_SCALE;
	    float sw = w * scale;
	    float sh = ( h - ascent ) * scale;

        Bounds3f textBounds( Vector3f( 0.0f, ascent * -scale, -VRMenuSurface::Z_BOUNDS ), Vector3f( sw, sh, VRMenuSurface::Z_BOUNDS ) );
        Vector3f trans( 0.0f );
        if ( FontParms.CenterHoriz )
        {
            trans -= Vector3f( sw * 0.5f, 0.0f, 0.0f );
        }
        if ( FontParms.CenterVert )
        {
            trans -= Vector3f( 0.0f, sh * 0.5f, 0.0f );
        }
        if ( FontParms.CenterHoriz || FontParms.CenterVert )
        {
            textBounds.Translate( trans );
        }
        TextLocalBounds = Bounds3f::Transform( GetTextLocalPose(), textBounds );
