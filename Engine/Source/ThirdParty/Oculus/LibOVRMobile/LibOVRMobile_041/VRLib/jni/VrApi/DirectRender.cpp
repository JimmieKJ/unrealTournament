	...
	if ( FrontBufferExtensionPresent() )
	{
		...
		if ( setFrontBufferID != NULL )
		{
			...
			*/
			...
			if ( env->ExceptionOccurred() )
			{
				LOG( "Exception: egl_GVR_FrontBuffer failed" );
				env->ExceptionClear();
...
void DirectRender::InitForCurrentSurface( JNIEnv * jni, int buildVersionSDK_ )
{
	...
	else if ( ( gpuType & GPU_TYPE_ADRENO ) != 0 )
	{
		...
		else
		{
			...
			if ( samples != 0 )
			{
				LOG( "PERFORMANCE WARNING: frame buffer uses MSAA - turn off MSAA!" );
				tilerControl = FB_TILED_RENDERING;
			}
			else
			{
				// TESTING: Enable TILED_RENDERING - current implementation of FB_BINNING_CONTROL
				// may be causing judder.
#if 0
...
{
	return gvrFrontbufferExtension;
}
