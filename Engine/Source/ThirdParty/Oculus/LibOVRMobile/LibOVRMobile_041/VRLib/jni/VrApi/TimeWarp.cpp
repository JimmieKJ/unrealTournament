...
Matrix4f CalculateTimeWarpMatrix2( const Quatf &inFrom, const Quatf &inTo )
{
...
#if 0
	{
		...
		float det = lastSensorMatrix.Determinant();
		if ( fabs( det ) < Mathf::SmallestNonDenormal )
		{
			DROIDLOG( "TimeWarpCrash", "from = (%.2f, %.2f, %.2f, %.2f )", from.x, from.y, from.z, from.w );
			DROIDLOG( "TimeWarpCrash", "to = (%.2f, %.2f, %.2f, %.2f )", to.x, to.y, to.z, to.w );
			LogMatrix( "lastSensorMatrix", lastSensorMatrix );
			//DROID_ASSERT( !"lastSensorMartrix det is 0", "TimeWarpCrash" );
		}

		Matrix4f combinedMatrix ( lastSensorMatrix.Inverted() * lastViewMatrix );
		det = combinedMatrix.Determinant();
		if ( fabs( det ) < Mathf::SmallestNonDenormal )
		{
			DROIDLOG( "TimeWarpCrash", "from = (%.2f, %.2f, %.2f, %.2f )", from.x, from.y, from.z, from.w );
			DROIDLOG( "TimeWarpCrash", "to = (%.2f, %.2f, %.2f, %.2f )", to.x, to.y, to.z, to.w );
			LogMatrix( "combinedMatrix", combinedMatrix );
			//DROID_ASSERT( !"combinedMatrix det is 0", "TimeWarpCrash" );
		}
	}
#endif
...
void TimeWarpLocal::WarpThread()
{
	...
	for ( double vsync = 0; ; vsync++ )
	{
		...
		WarpToScreen( vsync,
				InitParms.Screen->IsFrontBuffer() ? spAsyncFrontBufferPortrait
	...
	pthread_cond_broadcast( &ShutdownCondition );
}
...
TimeWarpLocal::~TimeWarpLocal()
{
	...
	if ( warpThread != 0 )
	{
		...
		LOG( "ShutdownAcknowledged, TimeWarp thread has exited." );

		// Attach the windowSurface to the calling context again.
		if ( eglMakeCurrent( eglDisplay, InitParms.Screen->windowSurface,
				InitParms.Screen->windowSurface, eglShareContext ) == EGL_FALSE )
		{
	...
	LastSwapVsyncCount( 0 )
{
	...
	pthread_mutex_init( &ShutdownMutex, NULL /* default attributes */ );
	pthread_cond_init( &ShutdownCondition, NULL /* default attributes */ );

	LOG("-------------------- Startup() --------------------");

	if ( initParms.Screen->windowSurface == EGL_NO_SURFACE )
	{
		FAIL( "initParms.Screen->windowSurface == EGL_NO_SURFACE" );
	}
...
void TimeWarpLocal::WarpThreadInit()
{
	...
	LOG( "eglMakeCurrent on %p", InitParms.Screen->windowSurface );
	if ( eglMakeCurrent( eglDisplay, InitParms.Screen->windowSurface,
			InitParms.Screen->windowSurface, eglWarpContext ) == EGL_FALSE )
	{
		FAIL( "eglMakeCurrent failed: %s", EglErrorString() );
	}
...
static void BindEyeTextures( const warpSource_t & currentWarpSource, const ScreenEye eye, const GlTexture loadingTexture )
{
	...
	if ( currentWarpSource.WarpParms.WarpProgram == WP_MASKED_PLANE || currentWarpSource.WarpParms.WarpProgram == WP_CHROMATIC_MASKED_PLANE )
	{
		...
		const swapProgram_t &	swap )
{
	...
	InitParms.Screen->GetScreenResolution( screenWidth, screenHeight );
	...
	for ( ScreenEye eye = SCREENEYE_LEFT; eye <= SCREENEYE_RIGHT; eye = (ScreenEye)((int)eye+1) )
	{
		...
		InitParms.Screen->BeginDirectRendering( eye * screenWidth/2, 0, screenWidth/2, screenHeight );
		...
		InitParms.Screen->EndDirectRendering();

		LogEyeWarpGpuTime.End( eye );

		const double justBeforeFinish = TimeInSeconds();
		if ( InitParms.Screen->IsFrontBuffer() )
		{
			GL_Finish();
		}
...
void TimeWarpLocal::WarpToScreenSliced( const double vsyncBase )
{
	...
	InitParms.Screen->GetScreenResolution( screenWide, screenTall );
	...
	for ( int screenSlice = 0; screenSlice < NUM_SLICES_PER_SCREEN; screenSlice++ )
	{
		...
		InitParms.Screen->BeginDirectRendering( sliceSize*screenSlice, 0, sliceSize, screenTall );
...
void TimeWarpLocal::CreateFrameworkGraphics()
{
	...
	// this will use default search paths. If we ever had per-app paths, the search paths 
	// would need to be passed in when timewarp is initialized.
	SearchPaths sp;	
	...
	{	// If we have an explicit distortion file request, use that.
		String relPath;
		sp.ToRelativePath( InitParms.DistortionFileName, relPath );
		String fullPath;
		if ( sp.GetFullPath( relPath, fullPath ) )
		{
			MemBufferFile explicitFile( fullPath );
		...
		if ( buf.Buffer == NULL ) 
		{
			FAIL( "Distortion file '%s' failed to load", relPath.ToCStr() );
		}
	}
	else
    ...
    warpPrograms[ WP_SIMPLE ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
    	...
    	,
			...
			"void main()\n"
			"{\n"
			"	gl_FragColor = intensity * texture2D(Texture0, oTexCoord);\n"
			"}\n"
    	);

	warpPrograms[ WP_MASKED_PLANE ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
    	...
    	,
			...
			"		gl_FragColor = intensity * mix( color1, color0, color0.w );\n"	// pass through destination alpha
			"	}\n"
			"}\n"
    	);

	warpPrograms[ WP_MASKED_PLANE_EXTERNAL ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
			"uniform mediump mat4 Texm3;\n"
    ...
    warpPrograms[ WP_MASKED_CUBE ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
			"uniform mediump mat4 Texm3;\n"
    	...
    	,
			...
			"	gl_FragColor = vec4( intensity * mix( color1.xyz, color0.xyz, color0.w ), 1.0);\n"	// pass through destination alpha
			"}\n"

    	);

    warpPrograms[ WP_CUBE ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
			"uniform mediump mat4 Texm3;\n"
    ...
    warpPrograms[ WP_LOADING_ICON ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
    	...
    	,
			...
			"void main()\n"
			"{\n"
			"	lowp vec4 color = intensity * texture2D(Texture0, oTexCoord);\n"
	...
	warpPrograms[ WP_MIDDLE_CLAMP ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
    	...
    	,
			...
			"void main()\n"
			"{\n"
			"	gl_FragColor = intensity * texture2D(Texture0, vec2( clamp( oTexCoord.x, TexClamp.x, TexClamp.y ), oTexCoord.y ) );\n"
			"}\n"
    	);

	warpPrograms[ WP_CHROMATIC ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
			"uniform mediump mat4 Texm3;\n"
	...
	warpPrograms[ WP_CHROMATIC_MASKED_PLANE ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
			"uniform mediump mat4 Texm3;\n"
	...
	warpPrograms[ WP_CHROMATIC_MASKED_PLANE_EXTERNAL ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
			"uniform mediump mat4 Texm3;\n"
    ...
    warpPrograms[ WP_CHROMATIC_MASKED_CUBE ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
			"uniform mediump mat4 Texm3;\n"
    	...
    	,
			...
			"	gl_FragColor = vec4( intensity * mix( color1, color0.xyz, color0.w ), 1.0);\n"	// pass through destination alpha
			"}\n"

    	);

    warpPrograms[ WP_CHROMATIC_CUBE ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
			"uniform mediump mat4 Texm3;\n"
    ...
    warpPrograms[ WP_CHROMATIC_LOADING_ICON ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
	...
	warpPrograms[ WP_CHROMATIC_MIDDLE_CLAMP ] = BuildWarpProg(
			"uniform mediump mat4 Mvpm;\n"
			"uniform mediump mat4 Texm;\n"
			"uniform mediump mat4 Texm2;\n"
		...
		const int swapOptions, const bool drawTimingGraph )
{
	...
	if ( swapOptions & SWAP_OPTION_DRAW_CALIBRATION_LINES )
	{
		...
		int width, height;
		InitParms.Screen->GetScreenResolution( width, height );
