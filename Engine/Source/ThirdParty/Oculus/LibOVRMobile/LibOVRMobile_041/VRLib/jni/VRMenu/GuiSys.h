...
public:
	...
	virtual void		Frame( App * app, VrFrame const & vrFrame, OvrVRMenuMgr & menuMgr, 
                                BitmapFont const & font , BitmapFontSurface & fontSurface ) = 0;
	...
	// called when re-orient is done from globalmenu or mount on 
	virtual void		RepositionMenus( App * app ) = 0;

    // Add a new menu that can be opened to receive events
    virtual void        AddMenu( VRMenu * menu ) = 0;
	// Remove a menu that was previously added
	virtual void		RemoveMenu( VRMenu * menu ) = 0;
