...
public:
	...
	void							Render( OvrVRMenuMgr & menuMgr, Matrix4f const & mvp, SubmittedMenuObject const & sub ) const;
...
private:
	void							CreateImageGeometry(  int const textureWidth, int const textureHeight, ContentFlags_t const content );
...
public:
    ...
    bool                        IntersectRay( Vector3f const & localStart, Vector3f const & localDir, Bounds3f const & bounds, 
...
private:
    ...
    mutable Bounds3f            TextLocalBounds;// cached local bounds for the text
...
private:
	...
	virtual void				RenderSurface( OvrVRMenuMgr & menuMgr, Matrix4f const & mvp, 
	...
	// hit by the ray. The ray should be in parent-local space - for the current root menu this is 
	// always world space.
	virtual bool				HitTest_r( App * app, OvrVRMenuMgr & menuMgr, BitmapFont const & font, Posef const & parentPose, 
                                        Vector3f const & rayStart, Vector3f const & rayDir,  ContentFlags_t const testContents, 
                                        HitTestResult & result ) const;
