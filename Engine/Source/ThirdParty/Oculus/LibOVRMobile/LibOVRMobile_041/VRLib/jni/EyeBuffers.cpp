...
void EyeBuffer::Allocate( const EyeParms & bufferParms, multisample_t multisampleMode )
{
	...
	{
		glTexImage2D( GL_TEXTURE_2D, 0, GL_RGB, bufferParms.resolution, bufferParms.resolution, 0,
				GL_RGB, GL_UNSIGNED_SHORT_5_6_5, NULL );
	}
	else if ( bufferParms.colorFormat == COLOR_8888_sRGB )
	{
		glTexImage2D( GL_TEXTURE_2D, 0, GL_SRGB8_ALPHA8, bufferParms.resolution, bufferParms.resolution, 0,
				GL_RGBA, GL_UNSIGNED_BYTE, NULL );
	}
	else
	{
		glTexImage2D( GL_TEXTURE_2D, 0, GL_RGBA8, bufferParms.resolution, bufferParms.resolution, 0,
				GL_RGBA, GL_UNSIGNED_BYTE, NULL );
	}
	...
	const GLenum depthFormat = ( bufferParms.depthFormat == DEPTH_24 ) ? GL_DEPTH_COMPONENT24_OES : GL_DEPTH_COMPONENT16;
	...
	if ( multisampleMode == MSAA_RENDER_TO_TEXTURE )
	{
		...
		glRenderbufferStorageMultisampleIMG_( GL_RENDERBUFFER,bufferParms. multisamples,
    			depthFormat, bufferParms.resolution, bufferParms.resolution );
	...
	else if ( multisampleMode == MSAA_BLIT )
	{
		...
		const GLuint fmt = ( bufferParms.colorFormat == COLOR_565 ) ?  GL_RGB565 : GL_RGBA8;
		glRenderbufferStorageMultisample_( GL_RENDERBUFFER, bufferParms.multisamples, fmt, bufferParms.resolution, bufferParms.resolution );
	...
	glScissor( 0, 0, bufferParms.resolution, bufferParms.resolution );
	glViewport( 0, 0, bufferParms.resolution, bufferParms.resolution );
...
void EyeBuffers::EndRenderingEye( const int eyeNum )
{
	...
	if ( eye.SyncObject )
	{
		eglDestroySyncKHR_( eglGetCurrentDisplay(), eye.SyncObject );
	}

	// Force the GPU to begin drawing this eye buffer.
	// This sync object will also be used to detect completion.
	eye.SyncObject = eglCreateSyncKHR_( eglGetCurrentDisplay(), EGL_SYNC_FENCE_KHR,
			NULL );
	if ( sync == EGL_NO_SYNC_KHR )
	{
		LOG( "eglCreateSyncKHR returned EGL_NO_SYNC_KHR" );
	}

	const EGLint wait = eglClientWaitSyncKHR_( eglGetCurrentDisplay(), eye.SyncObject,
			EGL_SYNC_FLUSH_COMMANDS_BIT_KHR, 0 );
	if ( wait == EGL_FALSE )
	{
		LOG( "eglClientWaitSyncKHR returned EGL_FALSE" );
	}
	//LOG( "Creating syncObject %p on SwapCount %i eye %i", eye.SyncObject, (int)SwapCount, eyeNum );
#else
	glFlush();
#endif
}
