...
void VRMenu::Init( OvrVRMenuMgr & menuMgr, BitmapFont const & font, float const menuDistance, VRMenuFlags_t const & flags, Array< VRMenuComponent* > comps /*= Array< VRMenuComponent* >( )*/ )
{
	...
	if ( root == NULL )
	{
		LOG( "RootHandle (%llu) is invalid!", RootHandle.Get() );
		return;
	}
        ...
        gazeCursorUserId_t const gazeUserId )
{
	...
	if ( NextMenuState != CurMenuState )
	{
		switch( NextMenuState )
		{
			...
			case MENUSTATE_OPEN:
				{
					LOG( "MENUSTATE_OPEN" );
					OpenSoundLimiter.PlaySound( app, "sv_release_active", 0.1 );
			...
			case MENUSTATE_CLOSED:
				{
					...
					CloseSoundLimiter.PlaySound( app, "sv_deselect", 0.1 );
	...
	if ( root != NULL )
	{
		...
		if ( app->IsPassThroughCameraEnabled() )
		{
			renderFlags |= VRMENU_RENDER_NO_DEPTH;
		}
		...
		switch( eventType )
		{
			...
			case KeyState::KEY_EVENT_SHORT_PRESS:
				if ( IsOpenOrOpening() )
				{
					...
					{
						app->StartPlatformUI( PUI_CONFIRM_QUIT );
						return true;
					}
