...
public:
	...
	virtual void				SendIntent( const char * package, const char * data );
	...
	const char *	launchIntent;		// empty string if launched without data
	const char *	packageCodePath;	// path to apk to open as zip and load resources
	...
	long long		lastCameraUpdateTime;
	...
	jmethodID		GetStaticMethodID( jclass cls, const char * name, const char * signature );
