...
public class VrActivity extends ActivityGroup implements SurfaceHolder.Callback {
	...
	public static native void nativeNewIntent(long appPtr, String path );
	...
	public static native SurfaceTexture nativeGetActivitySurfaceTexture(long appPtr);
	public static native void nativePopup(long appPtr, int width, int height, float seconds);
	public static native void nativeActivity(long appPtr, int width, int height, Activity activity);
	public static native void nativeActivityHide(long appPtr);
	public static native void nativeActivityRefresh(long appPtr);
	public static native void nativeVolumeChanged(long appPtr, int current, int max );
	...
	boolean createdNewWindow = false;	// for activity dialog
	...
	public void surfaceDestroyed(SurfaceHolder holder) {
		...
		nativeSurfaceChanged(appPtr, null);
	}
	...
	void adjustVolume(int direction) {
		...
		nativeVolumeChanged(appPtr, current, max );
	}
...
//	public boolean onTouchEvent( MotionEvent e ) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if ( uri != null ) {
			launchIntent = uri.getEncodedPath();
		} else if ( launchIntent == null ) {
			launchIntent = "";
		}

		// TODO: save the launch intent to let it be queried?
	...
	/**
	 * Here lies management not for toasts but for projected 2D Android UI's within our
	 * 3D space. The idea is to maintain the Activity lifecycle and redraw loop to allow
	 * for relatively seamless porting of 2D applications
	 */
	protected View projectedView;
	
	/**
	 * Recursively finds views that have been invalidated. If we find a view, terminate early.
	 * @param parentView
	 * @return
	 */
	protected static boolean anyDirtyViews( ViewGroup parentView )
	{
		...
		for( int i = 0; i < childCount; ++i )
		{
			...
			{
				if( anyDirtyViews( (ViewGroup)child ) )
					return true;
			}
	 ...
	 */
	protected Choreographer.FrameCallback frameCallback = new Choreographer.FrameCallback() {
		@Override
		public void doFrame(long frameTimeNanos)
		{
			if( projectedView != null )
			{
				if( anyDirtyViews( (ViewGroup) projectedView ) )
				{
	...
	public void createActivityDialog( View activityView, Activity activity )
	{
		// Find how big the view wants to be.
		activityView.measure(0, 0);
		activityView.layout(0, 0, activityView.getMeasuredWidth(),
				activityView.getMeasuredHeight());

		Log.v(TAG,
				"activty size:" + activityView.getWidth() + "x"
						+ activityView.getHeight());
		
		// Lazy init the activity texture
		if (activityTexture == null) {
			activityTexture = nativeGetActivitySurfaceTexture(appPtr);
			if (activityTexture == null) {
				Log.e(TAG, "nativeActivityDialog returned NULL");
				return; // not set up yet
			}
			activitySurface = new Surface(activityTexture);
		}
		
		activityTexture.setDefaultBufferSize(activityView.getWidth(),
				activityView.getHeight());
		...
		nativeActivity(appPtr, activityView.getWidth(), activityView.getHeight(), activity);
	}
	
	public void hideActivityDialog( )
	{
		nativeActivityHide(appPtr);
	}
	
	public void refreshActivityDialog( View activityView )
	{
		refreshActivityDialog( activityView, false );
	}
	
	public void refreshActivityDialog( View activityView, boolean force )
	{
		boolean needsRefresh = false || force;
		
		Canvas canvas = null;
		try {
			canvas = activitySurface.lockCanvas(null);
			canvas.drawColor(0, Mode.CLEAR);
			
			if( activityView.getVisibility() == View.VISIBLE || activityView.isDirty()  )
			{
			...
			Log.e(TAG, "lockCanvas threw exception: " + t.getMessage() );
		}
		
		// make sure we unlock the canvas		
		try {
			if ( canvas != null )
			{
				activitySurface.unlockCanvasAndPost(canvas);
			}
		} catch (Exception t2) {
			Log.e(TAG, "unlockCanvas threw exception: " + t2.getMessage() );
		}

		if( needsRefresh || createdNewWindow ) {
			nativeActivityRefresh(appPtr);
		}
	...
	public void startFlatActivityForResult(Intent intent, int requestCode,
			Bundle options)
	{
		startFlatActivityForResult( intent, requestCode );
	}
