...
class GPG_EXPORT GameServices::Builder {
 public:
  ...
  typedef std::function<
      void(MultiplayerEvent, std::string, MultiplayerInvitation)>
   ...
   */
  typedef std::function<
      void(MultiplayerEvent event, std::string, TurnBasedMatch)>
   ...
   * this will require Android users to have a G+ profile in order
   * to be able to sign in (on iOS, users must always have one).
   * Note: using this call requires using at least version 8.4.00 of the
