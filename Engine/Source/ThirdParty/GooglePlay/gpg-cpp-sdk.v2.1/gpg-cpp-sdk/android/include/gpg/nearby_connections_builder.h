...
class GPG_EXPORT NearbyConnections::Builder {
 public:
   ...
   * app must call it
   * prior to calling <code>Create</code>. The app may invoke the callback
   * multiple times.
   * For example, if a user gets a phone call and then returns to the app,
   * <code>NearbyConnections</code> will reinitialize and then call this
   * callback again.
   * Note that this callback must be called before you can use a
   * <code>NearbyConnections</code>
   * object.
   */
   ...
   * LogLevel)</code> with a
   * LogLevel of <code>INFO</code>.
   */
   ...
   * at the
   * specified log level. <code>min_level</code> specifies the minimum log level
   * at which the
   * app invokes the default callback.
   ...
   * LogLevel)</code> with
   * <code>OnLogCallback</code> set to <code>DEFAULT_ON_LOG_CALLBACK</code> and
   * a
   * <code>LogLevel</code> of <code>min_level</code>.
   */
