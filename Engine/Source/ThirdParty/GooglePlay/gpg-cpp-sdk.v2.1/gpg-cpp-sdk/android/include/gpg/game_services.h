...
class GPG_EXPORT GameServices {
 public:
   ...
   */
  struct FetchServerAuthCodeResponse {
    /**
  ...
  /**
   * Defines a callback type that receives a FetchServerAuthCodeResponse. This
   * callback type is provided to the <code>FetchServerAuthCode</code> function
   * below.
   * @ingroup Callbacks
   */
   ...
   *
   * Note: this function is only implemented for Android using at least version
   * 8.4.00 of the Google Play Services Java SDK.
   */
  void FetchServerAuthCode(std::string const &server_client_id,
                           FetchServerAuthCodeCallback callback);

  /**
   *  Synchronously retrieves the server auth code for this client.
   *  Leaving the timeout unspecified makes this function call equivalent
   *  to calling FetchServerAuthCodeResponse FetchServerAuthCodeBlocking(
   *  server_client_id, timeout), with timeout specified as 10 years.
   *
   *  Note: this function is only implemented for Android using at least version
   *  8.4.00 of the Google Play Services Java SDK.
   */
  FetchServerAuthCodeResponse FetchServerAuthCodeBlocking(
      std::string const &server_client_id);

  /**
   *  Synchronously retrieves the server auth code for this client.  Specify the
   ...
   */
  FetchServerAuthCodeResponse FetchServerAuthCodeBlocking(
      Timeout timeout, std::string const &server_client_id);
