...
void b2Fixture::Dump(int32 bodyIndex)
{
	...
	b2Log("    fd.filter.BlockingChannels = uint32(%X);\n", m_filter.BlockingChannels);
	b2Log("    fd.filter.TouchingChannels = uint32(%X);\n", m_filter.TouchingChannels);
	b2Log("    fd.filter.ObjectTypeAndFlags = uint32(%X);\n", m_filter.ObjectTypeAndFlags);
	b2Log("    fd.filter.UniqueComponentID = uint32(%X);\n", m_filter.UniqueComponentID);
	b2Log("    fd.filter.BodyIndex = uint32(%X);\n", m_filter.BodyIndex);
#else
