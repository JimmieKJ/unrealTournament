...
{
  REAL A=0.0f;
  for(PxU32 p=pcount-1,q=0; q<pcount; p=q++)
  {
...
void CTriangulator::_process(TU32Vector &indices)
{
    ...
    for (PxI32 m = 0, v = nv - 1; nv > 2;)
    {
        ...
        if (_snip(u, v, w, nv, V))
        {
			...
			if ( flipped )
			{
				indices.pushBack((physx::PxU32)a);
				indices.pushBack((physx::PxU32)b);
				indices.pushBack((physx::PxU32)c);
			}
			else
			{
				indices.pushBack((physx::PxU32)c);
...
{
    PxU32 n = mPoints.size();
    PxF64 A = 0.0f;
    for (PxU32 p = n - 1, q = 0; q < n; p = q++)
    {
...
bool CTriangulator::_snip(PxI32 u, PxI32 v, PxI32 w, PxI32 n, PxI32 *V)
{
    ...
    const TVec &A = mPoints[ (physx::PxU32)V[(physx::PxU32)u] ];
    const TVec &B = mPoints[ (physx::PxU32)V[(physx::PxU32)v] ];
    const TVec &C = mPoints[ (physx::PxU32)V[(physx::PxU32)w] ];
    ...
    for (p = 0; p < n; p++)
    {
        if ((p == u) || (p == v) || (p == w))
            continue;
        const TVec &P = mPoints[ (physx::PxU32)V[(physx::PxU32)p] ];
