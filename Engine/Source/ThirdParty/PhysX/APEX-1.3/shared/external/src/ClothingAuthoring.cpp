...
void ClothingAuthoring::removeMaterialFromLibrary(const char* libName, const char* matName)
{
	...
	{
		error = arrayHandle.swapArrayElements((physx::PxU32)deleteIndex, (physx::PxU32)numMaterials - 1);
		PX_ASSERT(error == NxParameterized::ERROR_NONE);
	}
#else // erase
	for (int i = deleteIndex + 1; i < numMaterials; i++)
	{
		error = arrayHandle.swapArrayElements((physx::PxU32)i - 1, (physx::PxU32)i);
		PX_ASSERT(error == NxParameterized::ERROR_NONE);
	}
#endif
...
unsigned int ClothingAuthoring::getNumMaterials(unsigned int libraryIndex) const
{
	...
	if (materialLibrary != NULL)
	{
		...
		return (physx::PxU32)numMaterials;
	}
			...
			for (int i = (int)mSimulation.actors.size(); i < mSimulation.actorCount; i++)
			{
				...
				for (size_t j = 0; j < mSimulation.assets.size(); j++)
				{
					...
					else
					{
						...
						physx::apex::NxClothingPlane* actorGroundPlane = NULL;
						if (mConfig.simulation.groundplaneEnabled && cActor->getClothSolverMode() == physx::apex::ClothSolverMode::v3)
						{
							physx::PxReal d = -mConfig.simulation.groundplane * mState.simulationValuesScale * 0.1f;
							physx::PxVec3 normal = mConfig.ui.zAxisUp ? PxVec3(0.0f, 0.0f, 1.0f) : PxVec3(0.0f, 1.0f, 0.0f);
...
void ClothingAuthoring::startCreateApexScene()
{
	...
	{
		for (physx::PxU32 i = 0; i < mMeshes.inputMesh->getNumSubmeshes(); i++)
		{
	...
	if (clothingAsset != NULL)
	{
		...
		for (PxU32 lod = 0; lod < numLods; ++lod)
		{
			...
			if (renderMeshAsset != NULL)
			{
				...
				char name[20];
				sprintf(name, "rma_%i_%i", mSimulation.assets.size(), lod);
				mSimulation.assets.back().renderMeshAssets[lod] = static_cast<physx::NxRenderMeshAsset*>(_mApexSDK->createAsset(rmaParamsCopy, name));
			}
		}
	...
	for (size_t i = 0; i < mSimulation.actors.size(); i++)
	{
		...
		for (size_t j = 0; j < mSimulation.assets.size(); j++)
		{
			...
			if (j < mSimulation.actors[i].actors.size())
			{
				...
				actor->setGraphicalLOD((physx::PxU32)mConfig.simulation.graphicalLod);
	...
	if (getAndClearGroundPlaneDirty() && mSimulation.physxScene != NULL)
	{
		...
		if (mConfig.simulation.groundplaneEnabled)
		{
			...
			for (size_t i = 0; i < mSimulation.actors.size(); i++)
			{
				for (size_t j = 0; j < mSimulation.actors[i].actors.size(); j++)
				{
					physx::apex::NxClothingActor* cActor = mSimulation.actors[i].actors[j];
					if (mSimulation.actors[i].actorGroundPlanes[j] != NULL)
					{
						mSimulation.actors[i].actorGroundPlanes[j]->release();
						mSimulation.actors[i].actorGroundPlanes[j] = NULL;
					}
					if (cActor && cActor->getClothSolverMode() == physx::apex::ClothSolverMode::v3)
					{
						physx::PxReal d = -mConfig.simulation.groundplane * mState.simulationValuesScale * 0.1f;
						physx::PxVec3 normal = mConfig.ui.zAxisUp ? PxVec3(0.0f, 0.0f, 1.0f) : PxVec3(0.0f, 1.0f, 0.0f);
						physx::PxPlane plane(normal, d);
...
{
	mSimulation.stepsUntilPause = (physx::PxU32)steps + 1; // always need one more
	...
	while (!OK)
	{
		...
		else if (mimp::gMeshImport)
		{
			...
			if (fph)
			{
				...
				unsigned int len = (physx::PxU32)ftell(fph);
				if (len > 0)
				{
...
void ClothingAuthoring::selectSubMesh(int subMeshNr, bool on)
{
	...
	{
		const Samples::TriangleSubMesh* submesh = mMeshes.inputMesh->getSubMesh((physx::PxU32)subMeshNr);
		if (submesh != NULL)
		{
			dirtyChanged |= submesh->selected != on;
		}
	...
	mMeshes.inputMesh->selectSubMesh((physx::PxU32)subMeshNr, on);
...
bool ClothingAuthoring::loadCustomPhysicsMesh(const char* filename)
{
	...
	while (!OK)
	{
		...
		else if (mimp::gMeshImport)
		{
			...
			{
				fseek(fph, 0L, SEEK_END);
				size_t len = (physx::PxU32)ftell(fph);
				if (len > 0)
				{
					fseek(fph, 0L, SEEK_SET);
...
int ClothingAuthoring::subdivideSubmesh(int subMeshNumber)
{
	...
	const Samples::TriangleSubMesh* submesh = mMeshes.inputMesh->getSubMesh((physx::PxU32)subMeshNumber);
	if (submesh != NULL)
	{
		...
		mMeshes.inputMesh->updatePaintingColors(Samples::PC_NUM_CHANNELS, mConfig.painting.valueMin, mConfig.painting.valueMax, (physx::PxU32)mConfig.painting.valueFlag, _mApexRenderDebug);
	...
	else
	{
		...
		for (physx::PxU32 i = 0; i < mMeshes.inputMesh->getNumSubmeshes(); i++)
		{
			...
			if (submesh->selected)
			{
				subdivideSubmesh((physx::PxI32)i);
			}
		}
	}
	...
	if (mMeshes.inputMesh != NULL && execute)
	{
		...
		if ((leftButton || rightButton) && mMeshes.painter->raycastHit())
		{
			        ...
			        mConfig.painting.channel == Samples::PC_COLLISION_DISTANCE)
			{
				...
				mMeshes.painter->paintFloat((physx::PxU32)mConfig.painting.channel, minimum, mConfig.painting.valueMax, paintValue);
			}
			else
			{
				unsigned int flag = (physx::PxU32)mConfig.painting.valueFlag;
				if (rightButton)
				{
					flag = ~flag;
				}

				mMeshes.painter->paintFlag((physx::PxU32)mConfig.painting.channel, flag, rightButton);
			}
...
void ClothingAuthoring::floodPainting(bool invalid)
{
	if (mMeshes.painter != NULL)
	{
		...
		{
			unsigned int flag = physx::PxU32(invalid ? ~mConfig.painting.valueFlag : mConfig.painting.valueFlag);
			mMeshes.painter->paintFlag((physx::PxU32)mConfig.painting.channel, flag, invalid);
		}
...
void ClothingAuthoring::smoothPainting(int numIterations)
{
	if (mMeshes.painter != NULL)
	{
		...
		mMeshes.painter->smoothFloatFast((physx::PxU32)mConfig.painting.channel, (physx::PxU32)numIterations);
...
void ClothingAuthoring::updatePaintingColors()
{
	if (mMeshes.inputMesh != NULL)
	{
		...
		mMeshes.inputMesh->updatePaintingColors(Samples::PC_LATCH_TO_NEAREST_SLAVE, 0, 0, (physx::PxU32)mConfig.painting.valueFlag, _mApexRenderDebug);
		mMeshes.inputMesh->updatePaintingColors(Samples::PC_LATCH_TO_NEAREST_MASTER, 0, 0, (physx::PxU32)mConfig.painting.valueFlag, _mApexRenderDebug);
...
void ClothingAuthoring::setAnimationTime(float time)
{
	...
	{
		Samples::SkeletalAnimation* anim = mMeshes.skeleton->getAnimations()[(physx::PxU32)physx::PxAbs(animation) - 1];
...
bool ClothingAuthoring::updateAnimation()
{
	...
	else
	{
		...
		jumped |= clampAnimation(mConfig.animation.time, true, mConfig.animation.loop, anims[(physx::PxU32)anim]->minTime, anims[(physx::PxU32)anim]->maxTime) && !mConfig.animation.continuous;
...
unsigned int ClothingAuthoring::generateCollisionVolumes(bool useCapsule, bool commandMode, bool dirtyOnly)
{
	...
	{
		const int boneIndex = mMeshes.collisionVolumes[(physx::PxU32)i].boneIndex;
		if ((bones[(physx::PxU32)boneIndex].dirtyParams && dirtyOnly) || (!bones[(physx::PxU32)boneIndex].manualShapes && !dirtyOnly))
		{
	...
	if (autoGeometry != NULL)
	{
		...
		{
			const Samples::SkeletalBone& source = bones[(physx::PxU32)i];
			...
			else
			{
				...
				mMeshes.skeleton->setBoneDirty((physx::PxU32)i, false);
			}
		...
		{
			addCollisionVolumeInternal(hulls[i], useCapsule && bones[(physx::PxU32)hulls[i]->mBoneIndex].allowPrimitives);
		}
	...
	if (useCapsule && mMeshes.skeleton != NULL && false)
	{
		for (size_t i = 0; i < mMeshes.collisionVolumes.size(); i++)
		{
			...
			if (mMeshes.skeleton->getBones()[(physx::PxU32)volume.boneIndex].allowPrimitives)
			{
				continue;
			}
...
ClothingAuthoring::CollisionVolume* ClothingAuthoring::addCollisionVolume(bool useCapsule, unsigned int boneIndex, bool createFromMesh)
{
	...
	if (!createFromMesh)
	{
		...
		volume.boneIndex = (physx::PxI32)boneIndex;
		volume.boneName = mMeshes.skeleton->getBones()[(physx::PxU32)boneIndex].name;
...
ClothingAuthoring::CollisionVolume* ClothingAuthoring::getCollisionVolume(int index)
{
	...
	{
		return &mMeshes.collisionVolumes[(physx::PxU32)index];
	}
...
void ClothingAuthoring::drawCollisionVolumes(bool wireframe) const
{
	...
	for (size_t i = 0; i < mMeshes.collisionVolumes.size(); i++)
	{
		...
		{
			const std::vector<Samples::SkeletalBone> &bones = mMeshes.skeleton->getBones();
			xform = bones[(physx::PxU32)volume.boneIndex].currentWorldPose * volume.shapeOffset;

			if (!bones[(physx::PxU32)volume.boneIndex].manualShapes)
			{
				inflation = bones[(physx::PxU32)volume.boneIndex].inflateConvex * mState.simulationValuesScale / 100.0f;    // a bit magic, I know
			}
			...
			selected = bones[(physx::PxU32)volume.boneIndex].selected;
		}
...
#else
			// sdbm
			unsigned long hash = (physx::PxU32)volume.boneIndex;
			while (*str)
			{
				int c = *str;
...
physx::apex::NxRenderMeshAssetAuthoring* ClothingAuthoring::getRenderMeshAsset(int index)
{
	...
	return mAuthoringObjects.renderMeshAssets[(physx::PxU32)index];
}
...
physx::apex::NxClothingPhysicalMesh* ClothingAuthoring::getClothMesh(int index, physx::apex::IProgressListener* progress)
{
	...
	return mAuthoringObjects.physicalMeshes[(physx::PxU32)index];
}
...
void ClothingAuthoring::simplifyClothMesh(float factor)
{
	for (size_t i = 0; i < mAuthoringObjects.physicalMeshes.size(); i++)
	{
		...
		mAuthoringObjects.physicalMeshes[i]->simplify(subdivSize, (physx::PxI32)maxSteps, -1, NULL);
	}
}
...
int ClothingAuthoring::getNumClothTriangles() const
{
	...
	{
		return (physx::PxI32)mAuthoringObjects.physicalMeshes[0]->getNumIndices() / 3;
	}
...
void ClothingAuthoring::simplifyIsoMesh(float factor, physx::apex::IProgressListener* progressParent)
{
	...
	if (factor == 0.0f)
	{
		mAuthoringObjects.isoMesh->simplify((physx::PxU32)mConfig.softbody.isoSimplifySubdivision, (physx::PxI32)SIMPLIFICATOR_MAX_STEPS, -1, &progress);
	}
	else
	{
		unsigned int numSteps = (physx::PxU32)(factor * mAuthoringObjects.isoMesh->getNumVertices());
		mAuthoringObjects.isoMesh->simplify(0, physx::PxMax(1, (physx::PxI32)numSteps), 5.0f, NULL);
	}
	progress.completeSubtask();
}
...
int ClothingAuthoring::getNumIsoSurfaceTriangles() const
{
	...
	{
		return (physx::PxI32)mAuthoringObjects.isoMesh->getNumIndices() / 3;
	}
...
int ClothingAuthoring::getNumTetrahedra() const
{
	...
	{
		return (physx::PxI32)mAuthoringObjects.physicalMeshes[0]->getNumIndices() / 4;
	}
...
physx::apex::NxClothingAssetAuthoring* ClothingAuthoring::getClothingAsset(physx::apex::IProgressListener* progress)
{
	...
	if (found != mState.materialLibraries.end())
	{
		...
		mAuthoringObjects.clothingAssetAuthoring->setMaterialLibrary(found->second, (physx::PxU32)materialIndex, false);
	}
	...
	if (!mMeshes.collisionVolumes.empty())
	{
		...
		*/
		...
		{
			const CollisionVolume& volume = mMeshes.collisionVolumes[i];
			const Samples::SkeletalBone& bone = mMeshes.skeleton->getBones()[(physx::PxU32)volume.boneIndex];
...
void ClothingAuthoring::setPaintingValueFlag(unsigned int flags)
{
	...
	{
		mDirty.workspace = true;
		mConfig.painting.valueFlag = (physx::PxI32)flags;
	...
	if (mConfig.animation.selectedAnimation != animation)
	{
		...
		if (mConfig.animation.selectedAnimation > 0)
		{
			if (!mConfig.animation.loop && mMeshes.skeleton != NULL)
			{
				...
				Samples::SkeletalAnimation* animation = mMeshes.skeleton->getAnimations()[(physx::PxU32)mConfig.animation.selectedAnimation - 1];
...
HACD::AutoGeometry* ClothingAuthoring::createAutoGeometry()
{
	...
	if (!meshVertices.empty() && (meshVertices.size() * 4) == boneIndices.size() && boneCount > 0)
	{
		...
		const std::vector<physx::PxU32> &meshIndices = inputMesh->getIndices();
...
void ClothingAuthoring::createRenderMeshAssets()
{
	...
	for (int graphicalLod = 0; graphicalLod < mConfig.cloth.numGraphicalLods; graphicalLod++)
	{
		...
		if (graphicalLod > 0)
		{
			...
			for (physx::PxU32 j = 0; j < subdividedMesh.getNumSubmeshes(); j++)
			{
				const Samples::TriangleSubMesh* submesh = subdividedMesh.getSubMesh(j);
				if (submesh != NULL && submesh->selected)
				{
					subdividedMesh.subdivideSubMesh((physx::PxI32)j, subdivision, mConfig.mesh.evenOutVertexDegrees);
				}
			}
		...
		for (size_t submeshIndex = 0, numSubmeshes = useThisMesh->getNumSubmeshes(); submeshIndex < numSubmeshes; submeshIndex++)
		{
			...
			if (submesh != NULL && submesh->selected)
			{
				...
				{
					if (texCoord < 4 && useThisMesh->getTexCoords((physx::PxI32)texCoord).size() == submeshDesc.m_numVertices)
					{
						...
						vertexBufferDesc.setSemanticData(semantic,
							&useThisMesh->getTexCoords((physx::PxI32)texCoord)[0],
...
void ClothingAuthoring::createClothMeshes(physx::apex::IProgressListener* progressParent)
{
	...
	for (physx::PxI32 i = 0; i < mConfig.cloth.numGraphicalLods; i++)
	{
		...
		physx::apex::NxClothingPhysicalMesh* physicalMesh = _mModuleClothing->createSingleLayeredMesh(
		            mAuthoringObjects.renderMeshAssets[(physx::PxU32)i],
		            physx::PxU32(mConfig.cloth.subdivide ? mConfig.cloth.subdivision : 0),
		            true, // mergeVerticest
		            mConfig.cloth.close,
		            &progress);
...
void ClothingAuthoring::createIsoMesh(physx::apex::IProgressListener* progress)
{
	...
	mAuthoringObjects.isoMesh = _mModuleClothing->createMultiLayeredMesh(renderMeshAsset, (physx::PxU32)mConfig.softbody.isoSurfaceSubdivision,
	                            (physx::PxU32)mConfig.softbody.keepBiggestMeshes, mConfig.softbody.discardInnerMeshes, progress);
...
void ClothingAuthoring::createClothingAsset(physx::apex::IProgressListener* progressParent)
{
	...
	unsigned int totalWork = (physx::PxU32)mConfig.cloth.numGraphicalLods * 2;
	ProgressCallback progress((physx::PxI32)totalWork, progressParent);
	...
	for (physx::PxU32 i = 0; i < mAuthoringObjects.physicalMeshes.size(); i++)
	{
		progress.setSubtaskWork(1, "combine graphical and physical mesh");

		const physx::PxU32 lod = mState.authoringState == AuthoringState::PhysicalClothMeshCreated ? (physx::PxU32)mConfig.cloth.numGraphicalLods - 1 - i : 0u;

		physx::PxU32 numPhysicalLods = (physx::PxU32)mMeshes.physicalLodValues.size();
...
void ClothingAuthoring::updateDeformableParameters()
{
	...
	if (mAuthoringObjects.clothingAssetAuthoring != NULL)
	{
		...
		mAuthoringObjects.clothingAssetAuthoring->setSimulationHierarchicalLevels((physx::PxU32)mConfig.deformable.hierarchicalLevels);
