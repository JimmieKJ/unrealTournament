...
namespace cloth
{
	...
	{
		return Scalar4i(int(floor(v.f4[0])), int(floor(v.f4[1])), 
			int(floor(v.f4[2])), int(floor(v.f4[3])));
	}
	...
	Scalar4i Gather<Scalar4i>::operator()(const Scalar4i* ptr) const
	{
		...
		return Scalar4i(base[index[0]] & mask[0], base[index[1]] & mask[1], 
			base[index[2]] & mask[2], base[index[3]] & mask[3]);
	}
