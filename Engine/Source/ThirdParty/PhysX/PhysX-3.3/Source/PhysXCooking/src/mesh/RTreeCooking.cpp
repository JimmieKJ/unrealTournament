...
struct PxBounds3V
{
	...
	// cast vector extents to PxVec3
	PX_FORCE_INLINE const PxVec3& getMinVec3() const { return (const PxVec3&)mn; }
	PX_FORCE_INLINE const PxVec3& getMaxVec3() const { return (const PxVec3&)mx; }
};
	...
	Array<PxU32>& resultPermute, RTreeCooker::RemapCallback* rc, PxReal sizePerfTradeOff01, PxMeshCookingHint::Enum hint)
{
	...
	Vec3V eps = V3Splat(FLoad(RTREE_INFLATION_EPSILON)); // AP scaffold: use PxTolerancesScale here?
...
struct SubSortQuick
{
	static const PxReal reductionFactors[RT_PAGESIZE-1];
		...
		PxBounds3V& subTreeBound, PxU32 level = 0)
	{
		...
		resultTree.resizeUninitialized(startNodeIndex+RT_PAGESIZE); // at each recursion level we add 4 nodes to the tree

		PxBounds3V childBound( (PxBounds3V::U()) ); // start off uninitialized for performance
		const PxI32 leftover = PxMax<PxI32>(PxI32(clusterSize - cluster4*(RT_PAGESIZE-1)), 0);
		PxU32 totalCount = 0;
		for (PxU32 i = 0; i < RT_PAGESIZE; i++)
		{
			...
			} else
			{
				...
				// leftover can be 0 if clusterSize<RT_PAGESIZE, this is generally rare, can happen for meshes with < RT_PAGESIZE tris
...
const PxReal SubSortQuick::reductionFactors[RT_PAGESIZE-1] = {0.8f, 0.7f, 0.6f};
...
struct SubSortSAH
{
		...
		Array<RTreeNodeNQ>& resultTree, PxU32& maxLevels, PxU32 level = 0, RTreeNodeNQ* parentNode = NULL)
	{
		...
		splitPos[RT_PAGESIZE-1] = clusterSize; // splitCount[n] is computed as splitPos[n+1]-splitPos[n], so we need to add this last value
		...
		// recurse on subpages
		PxU32 parentIndex = resultTree.size() - RT_PAGESIZE; // save the parentIndex as specified (array can be resized during recursion)
		for (PxU32 s = 0; s<RT_PAGESIZE; s++)
		{
	...
	PxReal sizePerfTradeOff01, PxMeshCookingHint::Enum hint)
{
	...
	PX_ASSERT(RT_PAGESIZE*sizeof(RTreeNodeQ) == sizeof(RTreePage)); // needed for nodePtrMultiplier computation to be correct
