...
bool Sc::ClothSim::addCollisionSphere(const ShapeSim* shape)
{
	...
	PxTransform trafo = core.getGlobalPose().transformInv(shape->getAbsPose());
...
bool Sc::ClothSim::addCollisionConvex( const ShapeSim* shape )
{
	...
	// current position here is before simulation
	Cm::Matrix34 trafo = core.getGlobalPose().transformInv(shape->getAbsPose()) * geometry.scale;
	...
	for(PxU32 i=0; i<numPlanes; ++i)
	{
		...
		planes.pushBack(PxPlane(normal, polygon.mPlane[3] - trafo.base3.dot(normal)));
	}
...
namespace
{
	...
	void gatherMeshVertices(const PxTriangleMesh& mesh, Ps::Array<PxVec3>& vertices, bool flipNormal)
	{
		...
		for(PxU32 i=0; i<numTriangles; ++i)
		{
			...
			if (flipNormal)
			{
				vertices.pushBack(meshVertices[indices[i*3+2]]);
				vertices.pushBack(meshVertices[indices[i*3+1]]);
			} else
			{
				vertices.pushBack(meshVertices[indices[i*3+1]]);
				vertices.pushBack(meshVertices[indices[i*3+2]]);
...
bool Sc::ClothSim::addCollisionHeightfield( const ShapeSim* shape )
{
	...
	Cm::Matrix34 trafo(core.getGlobalPose().transformInv(shape->getAbsPose()));

	trafo.base0 *= geometry.rowScale;
	trafo.base1 *= geometry.heightScale;
	trafo.base2 *= geometry.columnScale;
...
void Sc::ClothSim::updateRigidBodyPositions()
{
	...
	for (PxU32 i=0; i<mNumSpheres; ++i, ++shapeIt)
	{
		...
		const PxSphereGeometry& geometry = static_cast<const PxSphereGeometry&>(shape->getCore().getGeometry());
		PxVec3 p = core.getGlobalPose().transformInv(shape->getAbsPose().p);
	...
	for (PxU32 j= 0; j<mNumConvexes; ++j, ++shapeIt)
	{
		...
		for(PxU32 i=0; i<numPlanes; ++i)
		{
			...
			geometry.convexMesh->getPolygonData(i, polygon);
			PxVec3 normal = trafo.rotate(reinterpret_cast<const PxVec3&>(polygon.mPlane));
			planes.pushBack(PxPlane(normal, polygon.mPlane[3] - trafo.base3.dot(normal) - restOffset));
		}
	}
	...
	for (PxU32 i = 0; i<mNumMeshes; ++i, ++shapeIt)
	{
		...
		PxTransform meshPose = shape->getAbsPose();
		...
		if(geometry.triangleMesh->getTriangleMeshFlags() & PxTriangleMeshFlag::eHAS_16BIT_TRIANGLE_INDICES)
			gatherMeshVertices<PxU16>(*geometry.triangleMesh, curTriangles, geometry.scale.flipsNormal());
		else
			gatherMeshVertices<PxU32>(*geometry.triangleMesh, curTriangles, geometry.scale.flipsNormal());
	...
	for (PxU32 i = 0; i<mNumHeightfields; ++i, ++shapeIt)
	{
		...
		const PxHeightFieldGeometry& geometry = static_cast<const PxHeightFieldGeometry&>(shape->getCore().getGeometry());
		PxTransform clothPose = core.getGlobalPose();
		PxTransform heightfieldPose = shape->getAbsPose();
	...
	{
		if (Gu::intersectTriangleBox(bboxCenter, bboxScale, curTriangles[i], curTriangles[i+1], curTriangles[i+2]) ||
			Gu::intersectTriangleBox(bboxCenter, bboxScale, prevTriangles[i], prevTriangles[i+1], prevTriangles[i+2]))
		{
