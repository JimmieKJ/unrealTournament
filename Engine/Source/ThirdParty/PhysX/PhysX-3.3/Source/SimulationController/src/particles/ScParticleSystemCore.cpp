...
{	
	...
	if (isFluid)
	{
		...
		mLLParameter.flags |= PxvInternalParticleSystemFlag::eSPH;
	...
	mStandaloneData = PxsParticleData::create(maxParticles, perParticleRestOffset);
	if(perParticleRestOffset)
	{
...
void Sc::ParticleSystemCore::exportExtraData(PxSerializationContext& stream)
{
	...
	else
	{
		...
		PxsParticleData* tmp = PxsParticleData::create(particles, getParticleState().getWorldBoundsV());
...
{
	PX_ASSERT(!getSim());
	mStandaloneData = PxsParticleData::create(context);
}
//~PX_SERIALIZATION
...
PxvParticleSystemState&	Sc::ParticleSystemCore::getParticleState()
{ 
...
const PxvParticleSystemState& Sc::ParticleSystemCore::getParticleState() const 
{ 
	return getSim() ? getSim()->getParticleState() : *mStandaloneData; 
}
...
PxsParticleData* Sc::ParticleSystemCore::obtainStandaloneData()
{
	PX_ASSERT(mStandaloneData);
	PxsParticleData* tmp = mStandaloneData; 
...
void Sc::ParticleSystemCore::returnStandaloneData(PxsParticleData* particleData)
{
	PX_ASSERT(particleData && !mStandaloneData);
	mStandaloneData = (PxsParticleData*)particleData;
}
...
void Sc::ParticleSystemCore::getParticleReadData(PxParticleFluidReadData& readData) const
{
	...
	if (readData.validParticleRange > 0)
	{
		...
		if (mLLParameter.particleReadDataFlags & PxParticleReadDataFlag::eFLAGS_BUFFER)
			readData.flagsBuffer = PxStrideIterator<const PxParticleFlags>((PxParticleFlags*)particles.flags.ptr(), particles.flags.stride());
