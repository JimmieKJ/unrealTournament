...
PxsCCDContext::PxsCCDContext(PxsContext* context) : 
...
PxsCCDContext* PxsCCDContext::create(PxsContext* context)
{
	PxsCCDContext* dc = reinterpret_cast<PxsCCDContext*>(
		PX_ALLOC(sizeof(PxsCCDContext), PX_DEBUG_EXP("PxsCCDContext")));

	if(dc)
	{
		new(dc) PxsCCDContext(context);
	}
	return dc;
}
...
{
	if(atom)
		return atom->getPose() * atom->getCore().body2Actor.getInverse() * mShapeCore->transform;
	else
...
{
	return atom->getLastCCDTransform() * atom->getCore().body2Actor.getInverse() * mShapeCore->transform;
}

PxReal PxsCCDPair::sweepFindToi(PxsThreadContext& context, PxReal dt, PxU32 pass)
{
	...
	PxcSweepMethod sweepMethod = g_SweepMethodTable[g0][g1];
	...
	PxReal toi = sweepMethod(
		*ccdShape0, *ccdShape1, atom0, atom1, tm0, tm1, lastTm0, lastTm1, restDistance,
		sweepNormal, sweepPoint,
		mCm->mNpUnit.pairCache, context, mMinToi);
	...
	context.mContactBuffer.contact(mMinToiPoint, mMinToiNormal, 0.f, g0 == PxGeometryType::eTRIANGLEMESH || g0 == PxGeometryType::eHEIGHTFIELD? 
		mFaceIndex : PXC_CONTACT_NO_FACE_INDEX, g1 == PxGeometryType::eTRIANGLEMESH || g1 == PxGeometryType::eHEIGHTFIELD? mFaceIndex : PXC_CONTACT_NO_FACE_INDEX);
	...
	PxcNpWorkUnit& n = mCm->getWorkUnit();

	const PxsMaterialData& data0 = *n.materialManager->getMaterial(materialInfo.mMaterialIndex0);
	const PxsMaterialData& data1 = *n.materialManager->getMaterial(materialInfo.mMaterialIndex1);
...
void PxsCCDPair::updateShapes()
{
	if(mBa0)
	{
		...
		if(mBa0->mCCD->mUpdateCount != mCCDShape0->mUpdateCount)
		{
			...
			mCCDShape0->mShapeCore->geometry.computeBoundsWithCCDThreshold(origin, extents, tm0, NULL);
	...
	if(mBa1)
	{
		...
		if(mBa1->mCCD->mUpdateCount != mCCDShape1->mUpdateCount)
		{
			...
			PxVec3 origin, extents;
			mCCDShape1->mShapeCore->geometry.computeBoundsWithCCDThreshold(origin, extents, tm1, NULL);
...
PxReal PxsCCDPair::sweepEstimateToi()
{
	...
	if(g1 == PxGeometryType::eTRIANGLEMESH)
	{
		...
		PxF32 toi = physx::PxcSweepEstimateAnyShapeMesh(*ccdShape0, *ccdShape1, atom0, atom1, tm0, tm1, restDistance);
	...
	else if (g1 == PxGeometryType::eHEIGHTFIELD)
	{
		...
		PxF32 toi = physx::PxcSweepEstimateAnyShapeHeightfield(*ccdShape0, *ccdShape1, atom0, atom1, tm0, tm1, restDistance);
	...
	PxF32 toi = sweepAABBAABB(centreA, extentsA * 1.1f, centreB, extentsB * 1.1f, trA, trB);
	mMinToi			= toi;
	return toi;
}
...
bool PxsCCDPair::sweepAdvanceToToi(PxReal dt, bool clipTrajectoryToToi)
{
	...
	{
		if(thisPair->mCm->getWorkUnit().disableResponse)
		{
...
static const bool s_enableCCD = 1;
...
public:
	...
	virtual void runInternal()
	{
		...
		PxsThreadContext* threadContext = mContext->getThreadContext();
			...
			for (PxU32 iFront = islandStart; iFront < islandEnd; iFront++)
			{
...

				{
					...
					if(pair.mMinToi <= islandMinToi && 
						pair.mIsModifiable &&
						mContext->getCCDContactModifyCallback())
					{
				...
				if (advanced && pair.mMinToi <= 1.0f)
				{
					...
					if(!mDisableResweep && !pair.mCm->getWorkUnit().disableResponse)
					{
						void* a0 = pair.mBa0 == NULL ? NULL : (void*)pair.mBa0;
						void* a1 = pair.mBa1 == NULL ? NULL : (void*)pair.mBa1;

						for(PxU32 k = iFront+1; k < islandEnd; ++k)
						{
							PxsCCDPair& pair1 = *mCCDPairs[k];

							void* b0 = pair1.mBa0 == NULL ? (void*)pair1.mCCDShape0 : (void*)pair1.mBa0;
							void* b1 = pair1.mBa1 == NULL ? (void*)pair1.mCCDShape1 : (void*)pair1.mBa1;
...
// --------------------------------------------------------------
void PxsCCDContext::updateCCD(PxReal dt, PxBaseTask* continuation, bool disableResweep)
{
	...
	PxsThresholdStream& thresholdStream = mContext->getThresholdStream();
	thresholdStream.clear();  // clear force threshold report stream
	...
	if ((miCCDPass > 0 && mSweepTotalHits == 0) || (mContext->mNumFastMovingShapes == 0)) 
	{
		mSweepTotalHits = 0;
		updateCCDEnd();
		return;
	...
	//We can globally disable CCD with a static variable
	if (!s_enableCCD)
	{
		updateCCDEnd();
		return;
	...
	mCCDThreadContext = mContext->getThreadContext();
	...
	{
		CM_PROFILE_ZONE(mContext->getEventProfiler(), Cm::ProfileEventId::Sim::GetccdPair());
	
		Cm::BitMap::Iterator it(mContext->mActiveContactManager);
		for (PxU32 index = it.getNext(); index != Cm::BitMap::Iterator::DONE; index = it.getNext())
		{
			...
			if (CCD_SKIP_JOINT_VS_JOINT && isJoint0 && isJoint1)
				continue;
			...
			
			{
				...
				//We now create the CCD pair. These are used in the CCD sweep and update phases
				{
					...
					p.mG0 = static_cast<PxGeometryType::Enum>(cm->mNpUnit.geomType0);
					p.mG1 = static_cast<PxGeometryType::Enum>(cm->mNpUnit.geomType1);
					p.mCm = cm;
					...
					mContext->mSimStats.numCCDPairs[PxMin(p.mG0, p.mG1)][PxMax(p.mG0, p.mG1)] ++;
					#endif
		...
		{
			updateCCDEnd();
			mContext->putThreadContext(mCCDThreadContext);
			return;
		}
	}
	...
	//into islands and is also used to prioritize the pairs into their TOIs
	{
		...
		//Set up the threshold stream
		thresholdStream.reserve(Ps::nextPowerOfTwo(size));
	}
...
{	
	...
	PxsThresholdStream& thresholdStream = mContext->mThresholdStream;
		...
		for(PxU32 j = index; j < islandEnd; ++j)
		{
			...
			if(p.mIsEarliestToiHit)
			{
				...
				const bool createReports = 
					...
					&& ((p.mCm->mNpUnit.flags & PxcNpWorkUnitFlag::eDYNAMIC_BODY0 && PX_IR(static_cast<const PxsBodyCore*>(p.mCm->mNpUnit.rigidCore0)->contactReportThreshold) != 0x7f7fffff)	// PX_MAX_REAL
					|| (p.mCm->mNpUnit.flags & PxcNpWorkUnitFlag::eDYNAMIC_BODY1 && PX_IR(static_cast<const PxsBodyCore*>(p.mCm->mNpUnit.rigidCore1)->contactReportThreshold) != 0x7f7fffff)));	// PX_MAX_REAL
				...
				if(createReports)
				{
					...
					cp.internalFaceIndex0 = p.mFaceIndex;
					cp.internalFaceIndex1 = PXC_CONTACT_NO_FACE_INDEX;
					cp.separation = 0.0f;
					...
					PxU16 contactCount;
					PX_UNUSED(contactCount);
					PxU16 statusFlag = p.mCm->mNpUnit.statusFlags;
					PxsCCDContactHeader* ccdHeader = reinterpret_cast<PxsCCDContactHeader*>(p.mCm->mNpUnit.ccdContacts);
					if (writeCompressedContact(buffer.contacts, numContacts, *mCCDThreadContext, contactCount, contactStream,
												contactStreamSize, contactForces, numContacts*sizeof(PxReal), statusFlag, p.mCm->mNpUnit.materialManager,
												((p.mCm->mNpUnit.flags & PxcNpWorkUnitFlag::eMODIFIABLE_CONTACT) != 0), true, &matInfo, sizeof(PxsCCDContactHeader)) )
					{
						PxsCCDContactHeader* newCCDHeader = reinterpret_cast<PxsCCDContactHeader*>(contactStream);
						newCCDHeader->contactStreamSize = Ps::to16(contactStreamSize);
						newCCDHeader->isFromPreviousPass = 0;

						p.mCm->mNpUnit.ccdContacts = contactStream;	// put the latest stream at the head of the linked list since it needs to get accessed every CCD pass
																	// to prepare the reports
						p.mCm->mNpUnit.statusFlags = statusFlag;	// on success, the status should not change compared to previous collisions 
																	// (continuous or discrete) this frame, so just set it. On failure the stream 
