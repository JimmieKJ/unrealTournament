...
// by PrunerInterface
static PX_FORCE_INLINE PrunerPayload* translatePxU32ToPrunerPayload(PxU32 storedInAABBtree, PrunerPayload* objectsBase, PxU32 nbActiveObjects)
{
	if (storedInAABBtree == PxU32(-1))
	{
		Ps::getFoundation().error(PxErrorCode::eINTERNAL_ERROR, __FILE__, __LINE__, "Invalid index found in AABB-tree.");
		return NULL;
	}

	if (storedInAABBtree >= nbActiveObjects)
	{
		Ps::getFoundation().error(PxErrorCode::eINTERNAL_ERROR, __FILE__, __LINE__, "Invalid index found in AABB-tree.");
		return NULL;
	}
...
{
public:
	bool operator()(PrunerPayload* objects, PxU32 nbActiveObjects, const AABBTree& tree, const Test& test, PrunerCallback &visitor)
	{
		using namespace Cm;
		...
		while(stackIndex>0)
		{
			...
			while(test(center, extents))
			{
				if(node->isLeaf())
				{
					...
					if (!visitor.invoke(unusedDistance, translatePxU32ToPrunerPayload(*prunableIndex, objects, nbActiveObjects), 1))
						return false;
					break;
				}
...
public:
	bool operator()(
		PrunerPayload* objects, PxU32 nbActiveObjects, const AABBTree& tree,
		...
		PrunerCallback& pcb)
	{
		...
		while(stackIndex--)
		{
			...
			if(test.check<tInflate>(center, extents))	// TODO: try timestamp ray shortening to skip this
			{
				...
				prunableIndex = memFetchAsync<PxU32>(MemFetchPtr(node->getPrimitives(tree.getIndices())), 4, buf);
				memFetchWait(4);
				if (!pcb.invoke(md, translatePxU32ToPrunerPayload(*prunableIndex, objects, nbActiveObjects), 1))
					return false;
	...
	if(mAABBTree)
	{
		...
		case PxGeometryType::eBOX:
		{
			if (PxAbs(queryVolume.getPrunerWorldTransform().q.w) < 0.999999f)
			{
				...
				again = AABBTreeOverlap<Gu::OBBAABBTest>()(mPool.getObjects(), mPool.getNbActiveObjects(), *mAABBTree, test, pcb);
			}
			else
			{
				Gu::AABBAABBTest test(queryVolume.getPrunerInflatedWorldAABB());
				again = AABBTreeOverlap<Gu::AABBAABBTest>()(mPool.getObjects(), mPool.getNbActiveObjects(), *mAABBTree, test, pcb);
			}
		}
		break;
		case PxGeometryType::eCAPSULE:
		{
			...
			again = AABBTreeOverlap<Gu::CapsuleAABBTest>()(mPool.getObjects(), mPool.getNbActiveObjects(), *mAABBTree, test, pcb);
		}
		break;
		case PxGeometryType::eSPHERE:
		{
			...
			again = AABBTreeOverlap<Gu::SphereAABBTest>()(mPool.getObjects(), mPool.getNbActiveObjects(), *mAABBTree, test, pcb);
		}
		break;
...
PxAgain AABBPruner::sweep(const ShapeData& queryVolume, const PxVec3& unitDir, PxReal& inOutDistance, PrunerCallback& pcb) const
{
	...
	if(mAABBTree)
	{
		...
		again = AABBTreeRaycast<true>()(mPool.getObjects(), mPool.getNbActiveObjects(), *mAABBTree, aabb.getCenter(), unitDir, inOutDistance, extents, pcb);
	}
...
PxAgain AABBPruner::raycast(const PxVec3& origin, const PxVec3& unitDir, PxReal& inOutDistance, PrunerCallback& pcb) const
{
	...
	if (mAABBTree)
		again = AABBTreeRaycast<false>()(mPool.getObjects(), mPool.getNbActiveObjects(), *mAABBTree, origin, unitDir, inOutDistance, PxVec3(0.0f), pcb);
