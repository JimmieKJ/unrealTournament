...
#define SN_CHECK_PADDING    0


static char* convertToTiled(const char* src, int sizeOfSample, int nbColumns, int nbRows,
	                        int columnsPadded, int rowsPadded, int tilesU, int tilesV)
{
			...
			for(int v=0;v<HF_TILE_SIZE_V;v++)
			{
				for(int u=0;u<HF_TILE_SIZE_U;u++)
				{
						...
						(i < tilesV-1 || v < HF_TILE_SIZE_V-(rowsPadded - nbRows)))
					{
						...
						memcpy(samplesTiled + tileOffset*sizeOfSample, src + offset2*sizeOfSample, (size_t)sizeOfSample);
						//						samplesTiled[tileOffset] = src[offset2];
					}
					else
					{
						assert(tileOffset<columnsPadded*rowsPadded);
						memset(samplesTiled + tileOffset*sizeOfSample, 0, (size_t)sizeOfSample);
...
PxU64 Sn::ConvX::read64(const void*& buffer)
{
	...
	buffer = (const void*)(size_t(buffer) + sizeof(PxU64));
	PxU64 value = *buf64;
	output(value);
	return value;
}
	...
	for(int i=0; i < (maxNb + addVTablePtrShiftIteration); i++)
	{
		...
		if (i < nbSrcEntries)
		{
			displayMessage(PxErrorCode::eDEBUG_INFO, "\t0x%p\t%x\t%d\t%d\t%s", buffer + srcOffsetCheck, 
				(unsigned char)buffer[srcOffsetCheck], srcOffsetCheck, srcEntries[i].entry.mOffset, srcEntries[i].entry.mName);
			for (int byteCount = 1; byteCount < srcEntries[i].entry.mSize; ++byteCount) 
				...
				(unsigned char)buffer[srcOffsetCheck + byteCount], srcOffsetCheck + byteCount, srcEntries[i].entry.mOffset + byteCount);
		}
		...
		while(handlePadding)
		{
			...
			if(pad0 || pad1)
			{
				if(pad0)
				{
#if defined(PX_CHECKED) && SN_CHECK_PADDING
	                                        ...
	                                        int ptrSize_Src, int ptrSize_Dst)
{
	...
	displayMessage(PxErrorCode::eDEBUG_INFO, "+++++++++++++++++++++++++++++++++++++++++++++\n");
	displayMessage(PxErrorCode::eDEBUG_INFO, "\t0x%p\t%x\t\t\t%s", Address, (unsigned char)Address[0], entry.mName);
	for (int byteCount = 1; byteCount < ptrSize_Src*count; ++byteCount) 
		displayMessage(PxErrorCode::eDEBUG_INFO, "\t0x%p\t%x\t\t\t.", Address + byteCount, (unsigned char)Address[byteCount]);
			...
			for(int j=0;j<nbEntries;j++)
			{
				...
				else
				{
					...
					if(needsHFFiling && strcmp(ed.entry.mType, "PxHeightFieldSample")==0)
					{
						...
						// #### mNbSamples is not fixed, but it's only used during binary conversion anyway.
						//So it's a problem if you try to convert an already converted file, e.g.
						// a heightfield converted from PC to PS3 to PC again wouldn't work. But then again we don't even have the code to
						//"untile" the data from PS3 to PC anyway...

						// Convert to tiled
						const int sizeOfSample = 4;	// ### hardcoded
						char* const tiled = convertToTiled(srcData, sizeOfSample, hfData.columns, hfData.rows, hfData.columnsPadded,
							                               hfData.rowsPadded, hfData.tilesU, hfData.tilesV);
...
{
	PxU32 padding = Cm::getPadding(size_t(buffer), ALIGN_DEFAULT);
	...
	for(int i=0;i<nb;i++)
	{
		...
		if(!mc_classType_dst)
			return NULL;
		headerOffset += Cm::getPadding(size_t(mc_classType_dst->mSize), PX_SERIAL_ALIGN) + mc_classType_dst->mSize;	
	}
