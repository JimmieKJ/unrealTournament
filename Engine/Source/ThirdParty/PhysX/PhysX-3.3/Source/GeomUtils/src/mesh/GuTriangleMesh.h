...
// PxTriangleMesh
	...
	PX_PHYSX_COMMON_API virtual PxVec3*					getVerticesForModification();
	PX_PHYSX_COMMON_API virtual PxBounds3				refitBVH();
...
//	PX_FORCE_INLINE	PxU32						getTrigSharedEdgeFlags(PxU32 trigIndex) const			{ return mMesh.getTrigSharedEdgeFlags(trigIndex);		}
	...
	PX_FORCE_INLINE	void						computeTriangle(
													PxTriangle& worldTri, PxTriangleID triangleIndex,
													const Cm::Matrix34* worldMatrix = NULL, // use NULL to get a local space triangle
...
PX_FORCE_INLINE void Gu::TriangleMesh::computeTriangle(
	PxTriangle& worldTri, PxTriangleID triangleIndex,
	const Cm::Matrix34* worldMatrix, bool flipNormal,
	PxU32* PX_RESTRICT vertexIndices, PxU32* PX_RESTRICT adjacencyIndices) const
{
	...
	{
		worldTri.verts[0] = vertices[vref0];
		worldTri.verts[1] = vertices[vref1];
		worldTri.verts[2] = vertices[vref2];
	}
	...
	if(adjacencyIndices)
	{
		...
		{
			adjacencyIndices[0] = flipNormal ? mMesh.getAdjacencies()[triangleIndex*3 + 2] : mMesh.getAdjacencies()[triangleIndex*3 + 0];
			adjacencyIndices[1] = mMesh.getAdjacencies()[triangleIndex*3 + 1];
			adjacencyIndices[2] = flipNormal ? mMesh.getAdjacencies()[triangleIndex*3 + 0] : mMesh.getAdjacencies()[triangleIndex*3 + 2];
		}
		else
		{
