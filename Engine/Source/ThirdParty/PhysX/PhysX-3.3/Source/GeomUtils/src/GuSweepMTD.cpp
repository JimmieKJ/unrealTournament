...
static void midPhaseQueryMesh(const PxTriangleMeshGeometry& triMeshGeom, const PxTransform& pose, Gu::Box& bound, Container& tempContainer)
{
	GU_FETCH_MESH_DATA(triMeshGeom);

	Gu::Box vertexSpaceBox;
	computeVertexSpaceOBB(vertexSpaceBox, bound, pose, triMeshGeom.scale); // AP: possible optimization with identity scale
...
static void midPhaseQueryHF(const Gu::HeightFieldUtil& hfUtil, const PxTransform& pose, PxBounds3& bounds,  Container& tempContainer, PxU32 flags)
{
						  ...
						  const PxReal distance, const bool isDoubleSided,  PxSweepHit& hit)
{
	...
	for(PxU32 i=0; i<iterations; ++i)
	{
		...
		midPhaseQueryMesh(triMeshGeom, pose, capsuleBox, tempContainer);
		...
		for(PxU32 a = 0; a < nbBatches; ++a)
		{
			...
			for(PxU32 k=0; k<nbTrigs; k++)
			{
				...
				::getScaledTriangle(triMeshGeom, vertexToWorldSkew, triMeshGeom.scale.flipsNormal(), triangles[k], triangleIndex1);
...
bool physx::Gu::computeBox_TriangleMeshMTD(const PxTriangleMeshGeometry& triMeshGeom, const PxTransform& pose, const Gu::Box& _box, const PxTransform& boxTransform, const PxReal inflation, const PxReal distance, const bool isDoubleSided,  PxSweepHit& hit)
{
	...
	for(PxU32 i=0; i<iterations; ++i)
	{
		...
		midPhaseQueryMesh(triMeshGeom, pose, box, tempContainer);
		...
		for(PxU32 a = 0; a < nbBatches; ++a)
		{
			...
			for(PxU32 k=0; k<nbTrigs; k++)
			{
				//triangle vertext space
				const PxU32 triangleIndex1 = indices[startIndex+k];
				triMesh->computeTriangle(triangles[k], triangleIndex1, NULL, triMeshGeom.scale.flipsNormal());
				triangles[k].extraTriData = extraTrigData[triangleIndex1];
			}
						  ...
						  const PxReal distance, const bool isDoubleSided, PxSweepHit& hit)
{
	...
	for(PxU32 i=0; i<iterations; ++i)
	{
		...
		midPhaseQueryMesh(triMeshGeom, pose, hullOBB, tempContainer);
