...
{		
	...
	SweepCapsuleCallback(
		PxSweepHit& sweepHit, const Matrix34& worldMatrix, bool flipNormal_, PxReal distance, PxReal distCoeff_, bool doubleSide, 
		...
		:	MeshHitCallback<PxRaycastHit>(CallbackMode::eMULTIPLE),
			...
			hintFlags(hintFlags_), mostOpposingHitDot(2.0f), bestDist(PX_MAX_REAL),
			status(false), initialOverlapStatus(false), flipNormal(flipNormal_)
	{
	...
	{
		PxTriangle tmpt(	vertexToWorldSkew.transform(v0),
								vertexToWorldSkew.transform(v1),
								vertexToWorldSkew.transform(v2));

		if (flipNormal)
			Ps::swap<PxVec3>(tmpt.verts[1], tmpt.verts[2]);
		...
		if(!sweepCapsuleTriangles(	1, &tmpt, capsule, unitDir, PxMin(trueSweepDistance, trueSweepShrunkDistance+distEpsilon), NULL, hit.flags,
...
bool sweepCapsule_MeshGeom(GU_CAPSULE_SWEEP_FUNC_PARAMS)
{
	...
	if(!triMeshGeom.scale.isIdentity())
	{
		...
		flipNormal = triMeshGeom.scale.flipsNormal();
	} else
	{
	...
	SweepCapsuleCallback callback(sweepHit, poseWithScale, flipNormal, distance, distCoeff, isDoubleSided, inflatedCapsule, unitDir, hintFlags);
...
{		
	...
	SweepBoxMeshHitCallback(
		CallbackMode::Enum _mode, const Matrix34Padded& _meshToBox, bool _flipsNormal, PxReal distance, PxReal distCoeff_, bool doubleSide, 
		...
		const PxRaycastHit& meshHit, const PxVec3& lp0, const PxVec3& lp1, const PxVec3& lp2, PxReal& shrinkMaxT, const PxU32*)
	{
		if (!PX_IS_SPU && (hintFlags & PxHitFlag::ePRECISE_SWEEP))
		{
			...
			if (flipsNormal)
				Ps::swap<PxVec3>(currentTriangle.verts[1], currentTriangle.verts[2]);
		...
		else
		{
			...
			Vec3V triV1 = Vec3V_From_Vec4V(transformV(V4LoadU(&lp1.x), meshToBox));
			Vec3V triV2 = Vec3V_From_Vec4V(transformV(V4LoadU(&lp2.x), meshToBox));
			if (flipsNormal)
				Ps::swap<Vec3V>(triV1, triV2);
...
bool sweepBox_MeshGeom(GU_BOX_SWEEP_FUNC_PARAMS)
{
	...
	SweepBoxMeshHitCallback callback( // using eMULTIPLE with shrinkMaxT
		CallbackMode::eMULTIPLE, meshToBox, triMeshGeom.scale.flipsNormal(), distance, distCoeff, isDoubleSided, box, localDirDist, localDir, unitDir, hintFlags, inflation);
...
struct ConvexVsMeshSweepCallback : MeshHitCallback<PxRaycastHit>
{
		...
		const PxRaycastHit& hit, const PxVec3& av0, const PxVec3& av1, const PxVec3& av2, PxReal&, const PxU32*)
	{
		...
		PxVec3 v1 = mMeshScale * av1;
		PxVec3 v2 = mMeshScale * av2;
		if (mMeshScale.flipsNormal())
	...
	PxSweepHit& sweepHit, PxHitFlags hintFlags, const PxReal inflation)
{
	...
	PxVec3 meshSpaceSweepVector = meshScaling.getShape2VertexSkew().transform(meshPose.rotateInv(unitDir*distance));
	PxReal meshSpaceSweepDist = meshSpaceSweepVector.normalize();
	computeSweptBox(querySweptBox, hullOBB.extents, hullOBB.center, hullOBB.rot, meshSpaceSweepVector, meshSpaceSweepDist);
