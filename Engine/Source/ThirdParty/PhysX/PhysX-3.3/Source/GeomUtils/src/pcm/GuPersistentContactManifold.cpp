...
const PxF32 invalidateThresholds2[3] = { 0.5f, 0.5f, 0.75f	};
...
//between previous and current frame
const PxF32 invalidateQuatThresholds2[3] = { 0.9995f, 0.9997f, 0.9997f	};
...
Ps::aos::FloatV Gu::SinglePersistentContactManifold::reduceBatchContactsConvex(const MeshPersistentContact* manifoldContactExt, const PxU32 numContacts, PCMContactPatch& patch)
{
	...
	chosen[index] = true;
	clusters[0].index=(PxU32)index;
	clusters[0].origIndex = (PxU32)index;
	...
	//PX_ASSERT(chosen[index] == false);
	chosen[index] = true;
	clusters[1].index=(PxU32)index;
	clusters[1].origIndex = (PxU32)index;
	...
	chosen[index] = true;
	clusters[2].index=(PxU32)index;
	clusters[2].origIndex = (PxU32)index;
	...
	chosen[index1] = true;
	clusters[3].index = (PxU32)index1;
	clusters[3].origIndex = (PxU32)index1;
...
PxU32 Gu::SinglePersistentContactManifold::reduceContacts(MeshPersistentContact* manifoldPoints, PxU32 numPoints)
{
	...
	FloatV maxDis = max;
	PxI32 index = -1;
	MeshPersistentContact newManifold[4];
