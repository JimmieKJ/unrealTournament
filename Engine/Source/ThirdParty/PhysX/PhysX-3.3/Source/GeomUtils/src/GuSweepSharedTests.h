...
PX_FORCE_INLINE	void getScaledTriangle(
	const PxTriangleMeshGeometry& triGeom, const Cm::Matrix34& vertex2worldSkew, bool flipNormal, PxTriangle& triangle, PxTriangleID triangleIndex)
{
	Gu::TriangleMesh* tm = static_cast<Gu::TriangleMesh*>(triGeom.triangleMesh);
	tm->computeTriangle(triangle, triangleIndex, &vertex2worldSkew, flipNormal);
}
}
