...
//ML: this is needed because it seems NEON doesn't force the alignment on SIMD type, which cause some of the PCM unit tests fail
#if defined(PX_WINMODERN) && defined(PX_ARM)
#pragma warning(push)
#pragma warning(disable: 4324)	// Padding was added at the end of a structure because of a __declspec(align) value.
#endif
PX_ALIGN_PREFIX(16)
...
*/
PX_INLINE void MultiplePersistentContactManifold::fromBuffer(PxU8*  PX_RESTRICT buffer)
{
	using namespace Ps::aos;
	...
	if(buffer != NULL)
	{
		...
		MultiPersistentManifoldHeader*  PX_RESTRICT header = (MultiPersistentManifoldHeader* )buff;
		...
		for(PxU32 a = 0; a < numManifolds; ++a)
		{
			...
			SingleManifoldHeader*  PX_RESTRICT manHeader = (SingleManifoldHeader*)buff;
...
PX_INLINE void MultiplePersistentContactManifold::toBuffer(PxU8*  PX_RESTRICT buffer)
{
	...
	{
		SingleManifoldHeader* manHeader = (SingleManifoldHeader*)buff;
		buff += sizeof(SingleManifoldHeader);
...
#ifdef PX_PS3
PX_FORCE_INLINE PersistentContact* PX_CP_TO_PCP(ContactPoint* contactPoint)
{
	return physx::PxUnionCast<PersistentContact*, ContactPoint*>(contactPoint);
}

PX_FORCE_INLINE MeshPersistentContact* PX_CP_TO_MPCP(ContactPoint* contactPoint)
{
	return physx::PxUnionCast<MeshPersistentContact*, ContactPoint*>(contactPoint);
}
#else
#define PX_CP_TO_PCP(contactPoint)				((PersistentContact*)(contactPoint)) //this is used in the normal pcm contact gen
#define PX_CP_TO_MPCP(contactPoint)				((MeshPersistentContact*)contactPoint)//this is used in the mesh pcm contact gen
#endif
