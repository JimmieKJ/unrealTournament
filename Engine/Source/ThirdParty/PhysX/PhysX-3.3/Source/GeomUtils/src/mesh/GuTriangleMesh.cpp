...
struct RefitCallback : Gu::RTree::CallbackRefit
{
	...
	virtual void recomputeBounds(PxU32 index, Vec3V& aMn, Vec3V& aMx)
	{
...
PxBounds3 Gu::TriangleMesh::refitBVH()
{
	...
	if (mMesh.has16BitIndices())
	{
		RefitCallback<PxU16> cb(mMesh.mData.mVertices, static_cast<const PxU16*>(mMesh.mData.mTriangles));
		mMesh.mData.mCollisionModel.mRTree.refitAllStaticTree(cb, &meshBounds);
	} else
	{
		RefitCallback<PxU32> cb(mMesh.mData.mVertices, static_cast<const PxU32*>(mMesh.mData.mTriangles));
		mMesh.mData.mCollisionModel.mRTree.refitAllStaticTree(cb, &meshBounds);
	}

	// reset edge flags and remember we did that using a mesh flag (optimization)
	if ((mMesh.mData.mFlags & PxTriangleMeshFlag::eINTERNAL1) == 0)
	{
		mMesh.mData.mFlags |= PxTriangleMeshFlag::eINTERNAL1;
		const PxU32 nbTris = mMesh.getNumTriangles();
		for (PxU32 i = 0; i < nbTris; i++)
			mMesh.mData.mExtraTrigData[i] |= (Gu::ETD_CONVEX_EDGE_01|Gu::ETD_CONVEX_EDGE_12|Gu::ETD_CONVEX_EDGE_20);
	}

	mMesh.mData.mAABB = meshBounds;
	return meshBounds;
}
