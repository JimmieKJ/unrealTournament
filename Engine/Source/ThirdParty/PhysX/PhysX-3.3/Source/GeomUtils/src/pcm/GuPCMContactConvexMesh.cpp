...
public:
	...
	PCMConvexVsMeshContactGenerationCallback(
		...
		mGeneration(
			contactDistance, replaceBreakingThreshold, convexTransform, meshTransform, multiManifold, contactBuffer,
			polyData, polyMap, delayedContacts, convexScaling, idtConvexScale),
		mObbTriTest(box)
	{
	}
...
bool Gu::PCMContactConvexMesh(const Gu::PolygonalData& polyData, Gu::SupportLocal* polyMap, const Ps::aos::FloatVArg minMargin,
						const PxBounds3& hullAABB, const PxTriangleMeshGeometryLL& shapeMesh,
...
bool Gu::pcmContactConvexMesh(GU_CONTACT_METHOD_ARGS)
{
	...
	if(idtScaleConvex)
	{
		...
		return Gu::PCMContactConvexMesh(polyData, &convexMap, convexMargin, hullAABB, shapeMesh,transform0,transform1, contactDistance, contactBuffer,
			convexScaling,  meshScaling, idtScaleConvex, idtScaleMesh, multiManifold);
	}
	else
	{
...
bool Gu::pcmContactBoxMesh(GU_CONTACT_METHOD_ARGS)
{
	...
	return Gu::PCMContactConvexMesh(polyData, &boxMap, minMargin, hullAABB, shapeMesh,transform0,transform1, contactDistance, contactBuffer,
		idtScaling,  meshScaling, true, idtMeshScale, multiManifold);
}
