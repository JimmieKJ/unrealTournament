...
struct IntersectAnyVsMeshCallback : MeshHitCallback<PxRaycastHit>
{
	IntersectAnyVsMeshCallback(
		const Gu::RTreeMidphase& meshModel, const PxMat33& vertexToShapeSkew, bool flipNormal, LimitedResults* results)
...
// tSCB stands for templated on Sphere(tSCB=0), Capsule(tSCB=1) or Box(tSCB=2)
	...
	LimitedResults* results)
{
	...
	if (tSCB == eSPHERE)
	{
		IntersectAnyVsMeshCallback<tSCB, idtMeshScale> callback(hmd, vertexToShapeSkew, flipNormal, results);
	...
	else if (tSCB == eBOX)
	{
		...
		} else
		{
			...
			IntersectAnyVsMeshCallback<tSCB, idtMeshScale> callback(hmd, vertexToShapeSkew, meshScale.flipsNormal(), results);
...
struct ConvexVsMeshOverlapCallback : MeshHitCallback<PxRaycastHit>
{
			...
			mApplyScale		(0)
	{
		if (!identityScale)
		{
			mMeshScale = Ps::aos::Mat33V( // not done in initializer list for performance
			...
			mApplyScale = (meshScale.flipsNormal() ? PxU32(2) : PxU32(1));
		}
	...
	bool								mAnyHit;
	PxU32								mApplyScale; // 0 = identity, 1 = scale only, 2 = scale + flip normal (negative scale with neg determinant)
