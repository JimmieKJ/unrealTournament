...
void NpArticulationLink::releaseInternal()
{
	...
	if (npScene)
		npScene->getScene().removeRigidBody(mBody, true, false);
...
PxU32 NpArticulationLink::getChildren(PxArticulationLink** buffer, PxU32 bufferSize) const
{
	NP_READ_CHECK(NpActor::getOwnerScene(*this));

	const PxU32 size = mChildLinks.size();

	const PxU32 writeCount = PxMin(size, bufferSize);
	for(PxU32 i=0; i<writeCount; i++)
		buffer[i] = mChildLinks[i];

	return writeCount;
//	return Ps::dumpPointerArray((const void**)mChildLinks.begin(), mChildLinks.size(), (void**)buffer, bufferSize);
}
