...
//~PX_SERIALIZATION
	...
	PX_FORCE_INLINE const PxU32&	getRigidActorArrayIndex() const { return mIndex; }
...
{
	if ((getType() == PxActorType::eRIGID_STATIC) && (mShapeManager.getNbShapes() == 1))
	{
		Ps::getFoundation().error(PxErrorCode::eINVALID_PARAMETER, __FILE__, __LINE__, "RigidStatic::releaseShape: Can't release shape: A static actor can't be left with no shapes!");
		return;
	}
...
void NpRigidActorTemplate<APIClass>::attachShape(PxShape& shape)
{
	...
	GRB_EVENT(this->getScene(), GrbInteropEvent3, GrbInteropEvent3::PxActorAttachShape, static_cast<PxActor *>(this), static_cast<PxShape *>(&shape), 2);
}
...
void NpRigidActorTemplate<APIClass>::detachShape(PxShape& shape, bool wakeOnLostTouch)
{
	...
	GRB_EVENT(this->getScene(), GrbInteropEvent3, GrbInteropEvent3::PxActorDetachShape, static_cast<PxActor *>(this), static_cast<PxShape *>(&shape));
}
...
{
	PX_CHECK_AND_RETURN_VAL(!(ro.getActorFlags() & PxActorFlag::eDISABLE_SIMULATION), "Scene::resetFiltering(): Not allowed if PxActorFlag::eDISABLE_SIMULATION is set!", false);
	for(PxU32 i=0; i < shapeCount; i++)
	{
		...
		if (ra != this)
		{
			...
			PX_CHECK_AND_RETURN_VAL(found, "Scene::resetFiltering(): specified shape not in actor!", false);
		}
#endif
		PX_CHECK_AND_RETURN_VAL(static_cast<NpShape*>(shapes[i])->getScbShape().getFlags() & (PxShapeFlag::eSIMULATION_SHAPE | PxShapeFlag::eTRIGGER_SHAPE), "Scene::resetFiltering(): specified shapes not of type eSIMULATION_SHAPE or eTRIGGER_SHAPE!", false);
	}
