...
void NpConstraint::setConstraintFunctions(PxConstraintConnector& n,
										  const PxConstraintShaderTable& shaders)
{
	...
	if(mActor0)
	{
		...
		{
			bNeedUpdate = true;	
			NpActor::getFromPxActor(*mActor0).addConnector(NpConnectorType::eConstraint, this, "PxConstraint: Add to rigid actor 0: Constraint already added");
		}
	}
	...
	if(mActor1)
	{
		...
		{
			bNeedUpdate = true;	
			NpActor::getFromPxActor(*mActor1).addConnector(NpConnectorType::eConstraint, this, "PxConstraint: Add to rigid actor 0: Constraint already added");
		}
	}
...
void NpConstraint::updateConstants()
{
	...
	if( scbScene && !scbScene->isPhysicsRunning() && !scbScene->isPhysicsBuffering() )
		scbScene->getSceneVisualDebugger().updatePvdProperties( &mConstraint );
#endif
}
