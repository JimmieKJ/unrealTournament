...
namespace physx
{
	...
	#define UPDATE_PVD_PROPERTIES_OBJECT()	 {											\
		 ...
		 if(scene_ && !insertPending() && scene_->getSceneVisualDebugger().isConnected(true))	\
			 scene_->getSceneVisualDebugger().updatePvdProperties(this);	}
#else
	#define UPDATE_PVD_PROPERTIES_OBJECT() {}
#endif
...
//~PX_SERIALIZATION
		...
		PX_INLINE		bool					isSimulating()					const
						{
							const ControlState::Enum state = getControlState();
							return state == ControlState::eREMOVE_PENDING || // pending remove not possible if not buffered
								(state == ControlState::eIN_SCENE && mScene->isPhysicsRunning());
						}
