...
namespace Cm
{
	PX_CUDA_CALLABLE PX_INLINE PxBounds3 transformBounds34(const Cm::Matrix34& matrix, const PxBounds3& bounds)
	{
		if( bounds.isEmpty() )
			return bounds;
		else
	...
	public:
		...
		{
			return transformBounds34(getVertex2WorldSkew(Cm::Matrix34(shape2World)), bounds);
		}
