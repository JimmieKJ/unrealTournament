	...
	for (int i=0;i<UTGameState->PlayerArray.Num();i++)
	{
		...
		if (PS && !PS->bOnlySpectator)
		{
			if (TeamFilter < 0 || (PS->Team != NULL && PS->Team->GetTeamNum() == TeamFilter))
			{
				...
				UTHUDOwner->DrawString(FText::FromString(FString::Printf(TEXT("Ping %d"),int32(PS->ExactPing))), PingX, DrawY + (CellHeight * 0.25), ETextHorzPos::Left, ETextVertPos::Top, UTHUDOwner->MediumFont, DrawColor, CellScale * 0.5f);
