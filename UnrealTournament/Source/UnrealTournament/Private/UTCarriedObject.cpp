...
void AUTCarriedObject::OnOverlapBegin(AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult)
{
	...
	if (Character != NULL && !GetWorld()->LineTraceTest(OtherActor->GetActorLocation(), GetActorLocation(), ECC_Pawn, FCollisionQueryParams(FName(TEXT("PickupCheck")), false, OtherActor)))
	{
		TryPickup(Character);
	}
}
