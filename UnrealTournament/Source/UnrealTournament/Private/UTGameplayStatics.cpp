...
void UUTGameplayStatics::UTPlaySound(UWorld* TheWorld, USoundBase* TheSound, AActor* SourceActor, ESoundReplicationType RepType, bool bStopWhenOwnerDestroyed, const FVector& SoundLoc)
{
	if (TheSound == NULL)
	{
		UE_LOG(UT, Warning, TEXT("PlayReplicatedSound(): TheSound == None"));
	}
	else if (SourceActor == NULL && SoundLoc.IsZero())
	{
		UE_LOG(UT, Warning, TEXT("PlayReplicatedSound(): No source (SourceActor == None and SoundLoc not specified)"));
	}
	else
	{
		if (RepType >= SRT_MAX)
		{
			UE_LOG(UT, Warning, TEXT("PlayReplicatedSound(): Unexpected RepType"));
			RepType = SRT_All;
		}
