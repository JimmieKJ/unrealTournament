...
void SUTLoadoutWindow::CreateDesktop()
{
	CollectItems();
				...
				[			
					...
					[
						...
						[
							...
							[
								SNew(STextBlock)
								.Text(NSLOCTEXT("SUTLoadoutWindow","LoadoutTitle","Loadout"))
					...
					[
						...
						[
							...
							[
								...
								[
									SNew(STextBlock)
									.Text(this, &SUTLoadoutWindow::GetLoadoutTitle)
									.TextStyle(SUWindowsStyle::Get(),"UT.Dialog.TitleTextStyle")
								]
							...
							[
								...
								[
									...
									[
										...
										[
											SNew(SImage)
											.Image(this, &SUTLoadoutWindow::GetItemImage)
										]
									]
								]
								+SHorizontalBox::Slot()
								...
								[
									...
									.AutoWrapText( true )
									.Text(this, &SUTLoadoutWindow::GetItemDescriptionText)
								...
								[
									...
									[
										...
										[
											...
											.Text(NSLOCTEXT("SUTLoadoutWindow","Accept","Accept Loadout"))
											.TextStyle(SUWindowsStyle::Get(), "UT.TopMenu.Button.SmallTextStyle")
											.OnClicked(this, &SUTLoadoutWindow::OnAcceptClicked)
											.IsEnabled(this, &SUTLoadoutWindow::OnAcceptEnabled)
										]
									]
									...
									[
										...
										[
											...
											.TextStyle(SUWindowsStyle::Get(), "UT.TopMenu.Button.SmallTextStyle")
											.OnClicked(this, &SUTLoadoutWindow::OnCancelledClicked)
										]
									]
...
void SUTLoadoutWindow::CollectItems()
{
...
void SUTLoadoutWindow::TallyLoadout()
{
...
FText SUTLoadoutWindow::GetLoadoutTitle() const
{
	...
	return FText::Format(NSLOCTEXT("SUTLoadoutWindowu","LoadoutTitleFormat","Loadout... ${0}/${1}"), FText::AsNumber(TotalCostOfCurrentLoadout), CurrentCash);
}

const FSlateBrush* SUTLoadoutWindow::GetItemImage() const
{
...
FText SUTLoadoutWindow::GetItemDescriptionText() const
{
...
FReply SUTLoadoutWindow::OnAcceptClicked()
{
	if (PlayerOwner.IsValid() && PlayerOwner->PlayerController)
	{
		AUTPlayerController* PC = Cast<AUTPlayerController>(PlayerOwner->PlayerController);
...
FReply SUTLoadoutWindow::OnCancelledClicked()
{
	PlayerOwner->CloseLoadout();
	return FReply::Handled();
}

bool SUTLoadoutWindow::OnAcceptEnabled() const
{
	float CurrentCurrency = 0;
	if (PlayerOwner.IsValid() && PlayerOwner->PlayerController)
	{
...
void SUTLoadoutWindow::RefreshAvailableItemsList()
{
		...
		for (int32 Idx = 0; Idx < AvailableItems.Num(); Idx++)	
		{
			...
			AvailableItemsPanel->AddSlot(Col, Row).Padding(5.0, 5.0, 5.0, 5.0)
				[
					...
					[
						SNew(SUTButton)
						.OnClicked(this, &SUTLoadoutWindow::OnAvailableClick, Idx)
...
void SUTLoadoutWindow::RefreshSelectedItemsList()
{
		...
		for (int32 Idx = 0; Idx < SelectedItems.Num(); Idx++)
		{
			...
			SelectedItemsPanel->AddSlot(Col, Row).Padding(5.0, 5.0, 5.0, 5.0)
				[
					...
					[
						SNew(SButton)
						.OnClicked(this, &SUTLoadoutWindow::OnSelectedClick, Idx)
						.ButtonStyle(SUWindowsStyle::Get(), "UT.ComplexButton")
...
FReply SUTLoadoutWindow::OnAvailableClick(int32 Index)
{
	if (Index >= 0 && Index <= AvailableItems.Num())
	{
...
FReply SUTLoadoutWindow::OnSelectedClick(int32 Index)
{
...
FReply SUTLoadoutWindow::OnKeyUp(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
