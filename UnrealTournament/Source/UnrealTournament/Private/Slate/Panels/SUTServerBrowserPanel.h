...
#include "../Panels/SUTMatchPanel.h"
...
public:
	...
	virtual TSharedRef<SWidget> GenerateWidgetForColumn( const FName& ColumnName ) override
	{
		...
		if (ServerData.IsValid())
		{
			...
			else ColumnText = NSLOCTEXT("SUTServerBrowserPanel","UnknownColumnText","n/a");
		}
		else
		{
			ColumnText = NSLOCTEXT("SUTServerBrowserPanel","UnknownColumnText","n/a");
...
class UNREALTOURNAMENT_API SUTServerBrowserPanel : public SUTPanelBase
{
public:

	~SUTServerBrowserPanel();
...
protected:
	...
	TSharedPtr<class SUTMatchPanel> LobbyMatchPanel;
