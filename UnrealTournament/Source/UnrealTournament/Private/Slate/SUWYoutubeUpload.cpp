...
#include "SUWYoutubeUpload.h"
#include "SUWindowsStyle.h"

#if !UE_SERVER
void SUWYoutubeUpload::Construct(const FArguments& InArgs)
{
	...
	if (DialogContent.IsValid())
	{
		...
		[
			...
			[
				...
				.AutoWidth()
				[
					SNew(STextBlock).Text(NSLOCTEXT("SUWYoutubeUpload", "Uploading", "Uploading..."))
				]
			]
			+ SVerticalBox::Slot()
			...
			[
				...
				[
					SNew(SProgressBar)
					.Percent(this, &SUWYoutubeUpload::GetProgressCompression)
				]
			]
		];
	}
	...
	YoutubeResumableRequest->OnProcessRequestComplete().BindRaw(this, &SUWYoutubeUpload::YoutubeResumableSessionRequestComplete);
...
void SUWYoutubeUpload::YoutubeResumableSessionRequestComplete(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded)
{
	if (HttpResponse.IsValid())
	{
		if (HttpResponse->GetResponseCode() == 200)
		{
			...
			YoutubeUploadRequest->OnProcessRequestComplete().BindRaw(this, &SUWYoutubeUpload::YoutubeFileUploadRequestComplete);
			HttpRequest->OnRequestProgress().BindRaw(this, &SUWYoutubeUpload::YoutubeFileUploadProgress);
...
void SUWYoutubeUpload::YoutubeFileUploadProgress(FHttpRequestPtr InHttpRequest, int32 NumBytesSent, int32 NumBytesRecv)
{
	BytesUploaded = NumBytesSent;
}

void SUWYoutubeUpload::YoutubeFileUploadRequestComplete(FHttpRequestPtr HttpRequest, FHttpResponsePtr HttpResponse, bool bSucceeded)
{
	if (HttpResponse.IsValid())
	{
...
TOptional<float> SUWYoutubeUpload::GetProgressCompression() const
{
...
FReply SUWYoutubeUpload::OnButtonClick(uint16 ButtonID)
{
	OnDialogResult.ExecuteIfBound(SharedThis(this), UTDIALOG_BUTTON_CANCEL);
	GetPlayerOwner()->CloseDialog(SharedThis(this));
