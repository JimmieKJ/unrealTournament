...
public:
	...
	virtual bool CanPerformAction(const EFriendActionType::Type ActionType) override
	{
		switch (ActionType)
		{
			...
			case EFriendActionType::InviteToGame:
			{
				if (FriendItem->GetInviteStatus() == EInviteStatus::Accepted && FriendItem->IsOnline() && FriendItem->CanInvite())
				{
			...
			case EFriendActionType::Chat:
			{
				if (FriendItem->GetInviteStatus() == EInviteStatus::Accepted && FriendItem->IsOnline())
				{
					return true;
				}
				return false;
			...
			case EFriendActionType::MutePlayerRequest:
			{
				return !FriendsService->IsPlayerMuted(FriendItem->GetUniqueID());
			}
			case EFriendActionType::UnmutePlayerRequest:
			{
				return FriendsService->IsPlayerMuted(FriendItem->GetUniqueID());
