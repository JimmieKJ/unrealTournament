...
bool FHTTPChunkInstall::Tick(float DeltaSeconds)
{
	...
	case ChunkInstallState::ReadTitleFiles:
		{
			int LocalReads = 0;
			while (TitleFilesToRead.Num() > 0 && InstallSpeed != EChunkInstallSpeed::Paused && PendingReads.Num() < 15 && LocalReads < 15)
			{
...
void FHTTPChunkInstall::OSSEnumerateFilesComplete(bool bSuccess)
{
	...
	else
	{
		...
		ErrorDelegate.Broadcast(ErrorMsg);
	}
}
...
//	InstallerState = bSuccess ? ChunkInstallState::ReadComplete : ChunkInstallState::EnterOfflineMode;
	...
	if (PendingReads.Num() == 0 && TitleFilesToRead.Num() == 0)
	{
		if (bFirstRun)
		{
			ChunkMountTask.SetupWork(BPSModule, ContentDir, MountedPaks, ExpectedChunks);
	...
	else
	{
		...
		if (InstallService->HasError())
		{
			...
			case EBuildPatchInstallError::DownloadError:
				ErrorDelegate.Broadcast(LOCTEXT("ReadFiles", "Failure to download files."));
				break;
			case EBuildPatchInstallError::OutOfDiskSpace:
				ErrorDelegate.Broadcast(LOCTEXT("Diskspace", "No space on device."));
				break;
			case EBuildPatchInstallError::MoveFileToInstall:
				EndInstall();
				break;
			default:
				ErrorDelegate.Broadcast(LOCTEXT("ReadFiles", "Unknown Failure, retrying..."));
				EndInstall();
				break;
			}
		}
...
bool FHTTPChunkInstall::FilesHaveChanged(IBuildManifestPtr LocalManifest, IBuildManifestPtr RemoteManifest, int32 InInstallingChunkID)
{
	...
	InstallManifest->GetOutdatedFiles(CurrentManifest.ToSharedRef(), ChunkInstallDir, OutOfDateFiles);
...
void FHTTPChunkInstall::Initialize(const FString& InBaseUrl, const FString& InMcpConfig)
{
	InitialiseSystem(InBaseUrl, InMcpConfig);
	bEnabled = true;
}

void FHTTPChunkInstall::InitialiseSystem(const FString& InBaseUrl, const FString& InMcpConfig)
{
	...
	{
		OnlineTitleFile = OnlineTitleFileHttp = MakeShareable(new FOnlineTitleFileHttp(InBaseUrl, InMcpConfig));
	}
